...
#Region "SP"
    ...
    Private Const SP_select_analysis_Old As String = "select_analysis_Old"
...
#Region "Method"
    ...
    Private Sub fill_equity(ByVal table As DataTable)
        Try
            ...
            For Each drow As DataRow In table.DefaultView.ToTable(True, "script", "company", "eq").Rows
                newPos = True
                Dim dvtable As DataTable = New DataView(table, "script = '" & drow("script") & "' and company='" & drow("company") & "'", "", DataViewRowState.CurrentRows).ToTable
                For Each mrow As DataRow In maintable.Select("script = '" & drow("script") & "' and company='" & drow("company") & "'")
                    newPos = False
                    'Dim brate As Double = 0
                    'Dim srate As Double = 0
                ...
                If newPos = True Then
                    ...
                    For Each row As DataRow In eqmaster.Select("script='" & drow("script") & "' and symbol='" & drow("company") & "'")
                ...
                If Not ar.Contains(drow("script").ToString & drow("company").ToString) Then
                    ...
                    If count > 1 Then
                        ...
                        Else
                            dr = dtable.NewRow()
                        dr("strikes") = Val(drow("strikerate"))
                        If (table.Compute("sum(qty)", "script='" & drow("script") & "' And company='" & drow("company").ToString() & "' and entry_date < #" & Format(Today, "dd-MMM-yyyy") & "# ").ToString() = "") Then
                            dr("prqty") = 0
                        Else
                        ...
                        If (table.Compute("sum(qty)", "script='" & drow("script") & "' and entry_date >= #" & Format(Today, "dd-MMM-yyyy") & "#  And company='" & drow("company") & "'").ToString() = "") Then
                            dr("toqty") = 0
                        Else
    ...
    Public Function sel_analysis() As DataTable
        Try
            ...
            If gstr_ProductName = "OMI" Then
                data_access.Cmd_Text = SP_select_analysis_Old
            Else
    ...
    Public Sub UpdPreData_analysis1(ByVal entrydate As Date, ByVal PreSpot As Double, ByVal PreVol As Double, ByVal PreDelVal As Double, ByVal PreVegVal As Double _
                                    , ByVal PreTheVal As Double, ByVal preTotalMTM As Double, ByVal preGrossMTM As Double, ByVal preQty As Double, ByVal tokenno As Double)
        Try
            data_access.ParamClear()

            'data_access.AddParam("@entrydate", OleDbType.Date, 18, CDate(entrydate))
            ''data_access.AddParam("@preDate", OleDbType.Date, 18, CDate(Now.Date))
            'data_access.AddParam("@curSpot", OleDbType.Double, 18, Val(PreSpot))
            'data_access.AddParam("@curVol", OleDbType.Double, 18, Val(PreVol))

            'data_access.AddParam("@curDelVal", OleDbType.Double, 18, Val(PreDelVal))
            'data_access.AddParam("@curVegVal", OleDbType.Double, 18, Val(PreVegVal))
            'data_access.AddParam("@curTheVal", OleDbType.Double, 18, Val(PreTheVal))
            'data_access.AddParam("@curTotalMTM", OleDbType.Double, 18, Val(preTotalMTM))
            'data_access.AddParam("@curGrossMTM", OleDbType.Double, 18, Val(preGrossMTM))
            'data_access.AddParam("@Qty", OleDbType.Double, 18, Val(preQty))
            'data_access.AddParam("@tokenno", OleDbType.Double, 18, Val(tokenno))

            'data_access.Cmd_Text = sp_update_PreDate1
            Dim sql As String
            sql = "UPDATE analysis SET preDate = #" & CDate(entrydate).ToString("dd-MMM-yyyy") & "#, preSpot = " & PreSpot.ToString() & ", preVol = " & PreVol.ToString() & ", preDelVal = " & PreDelVal.ToString() & ", preVegVal = " & PreVegVal.ToString() & ", preTheVal = " & PreTheVal.ToString() & ", preTotalMTM = " & preTotalMTM.ToString() & ", preGrossMTM = " & preGrossMTM.ToString() & ", preQty = " & preQty.ToString() & " WHERE tokanno=" & tokenno.ToString() & ";"
            data_access.cmd_type = CommandType.Text
            data_access.Cmd_Text = sql

            data_access.ExecuteQuery(sql)
            data_access.cmd_type = CommandType.StoredProcedure
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    ...
    Public Sub insert_analysis(ByVal detable As DataTable)
        ...
        For Each drow As DataRow In detable.Rows
            ...
            data_access.AddParam("@preDate", OleDbType.Date, 18, CDate(IIf(IsDBNull(drow("preDate")), Now.Date, drow("preDate"))))
            'data_access.AddParam("@preDate", OleDbType.Date, 18, CDate(Now.Date))
