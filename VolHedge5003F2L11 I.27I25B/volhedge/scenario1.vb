...
Public Class scenario1
    Dim Market As MarketWatch
    Dim flgtabchange As Boolean = False
    Dim thr_Hash As New Hashtable
    ...
    Private Const cnstAnalysisScenario As String = "Analysis-Scenario"

    Dim TgAddSize As Long
    Dim TgRemSize As Long
    Dim ScenarioName As String
    Dim ObjSceDetail As New scenarioDetail
    Dim allcompanyFlag As New allcompany
    Dim div As Long = 1
    Dim str As String = ""
    Public markerstring As String
    Dim DSleft As Integer
    Dim DSTop As Integer
    Dim FrmDSMove As Boolean
    Dim flgload As Boolean = False

    Dim DragDS_X As Integer
    Dim DragDS_Y As Integer
    Dim trd As New trading
    Public temp1 As New DataTable



#Region "Variable"
    Public MarketSceName As String
    Dim Flgtxtcvol As Boolean = False
    Dim Flgtxtpvol As Boolean = False
    ...
    Dim ShowPanel3 As Boolean

    Dim KeyF4Togal As Boolean
    Dim KeyF3Togal As Boolean = True 'Panel Default True
    Dim KeyF7Togal As Boolean
    ...
    Public Shared dtprofit As New DataTable
    Public Shared dtdeltatable As New DataTable
    Public Shared dtgammatable As New DataTable
    Public Shared dtvegatable As New DataTable
    Public Shared dtthetatable As New DataTable
    Public Shared dtvolgatable As New DataTable
    Public Shared dtvannatable As New DataTable

    Dim static_Grid As New DataTable
    Public Shared rtable As DataTable
    Public Shared chartDataCol As New CChartCollection


#End Region
    'Public Sub ShowForm1(ByVal isAnalysis As Boolean, ByVal dt As DataTable)
    '    ScenarioName = "aa"
    '    scenariotable = dt
    '    flgmarketwatch = True

    '    If isAnalysis = True Then
    '        ScenarioName = cnstAnalysisScenario
    '        'ObjSceDetail.ScenarioName = ScenarioName
    '        'ObjSceDetail.Delete_scenario_Detail()
    '        If tbcomp.TabPages.Count > 0 Then
    '            Dim TAB = New TabPage
    '            TAB.Text = cnstAnalysisScenario
    '            TAB.Name = cnstAnalysisScenario
    '            TAB.Tag = cnstAnalysisScenario
    '            TAB.ToolTipText = cnstAnalysisScenario
    '            tbcomp.SelectedTab = tbcomp.TabPages(cnstAnalysisScenario)
    '            If tbcomp.SelectedTab Is Nothing Then
    '                'GoTo Add2
    '            End If
    '            If tbcomp.SelectedTab.Name = cnstAnalysisScenario Then

    '                'tbcomp.SelectedTab = tbcomp.TabPages(cnstAnalysisScenario)
    '            Else
    '                tbcomp.TabPages.Add(TAB)
    '                ScenarioName = TAB.Name
    '                tbcomp.SelectedTab = tbcomp.TabPages(cnstAnalysisScenario)
    '                'tbcomp.TabPages.Add(TAB)
    '                'ScenarioName = TAB.Name
    '                'tbcomp.SelectedTab = tbcomp.TabPages(cnstAnalysisScenario)
    ...
    '        LoadTag1(ScenarioName)
    '    Else
    '        If (ScenarioName Is Nothing Or ScenarioName = "") Or (ScenarioName = cnstAnalysisScenario) Then
    '            '                Dim TAB = New TabPage
    '            '                TAB.Text = cnstAnalysisScenario
    '            '                TAB.Name = cnstAnalysisScenario
    '            '                TAB.Tag = cnstAnalysisScenario
    ...
    '            '                If tbcomp.TabPages.Count > 0 Then
    '            '                    If tbcomp.TabPages.ContainsKey(TAB.Name) Then
    '            '                        MsgBox("Tag Allready Exist.(" & TAB.Name & ")", MsgBoxStyle.Information)
    '            '                        'Exit Sub
    '            '                    Else
    '            'Add2:
    '            '                        tbcomp.TabPages.Add(TAB)
    ...
    '            AddUserDefScenario("")
    '        End If
    '        If ScenarioName Is Nothing Then
    '            Exit Sub
    '        Else
    '            tbcomp.SelectedTab = tbcomp.TabPages(ScenarioName)
    '            ' ObjSceDetail.ScenarioName = ScenarioName
    '            LoadTag1(ScenarioName)
    '        End If
    ...
    'End Sub
    Public Sub ShowForm(ByVal isAnalysis As Boolean, ByVal MarketSceName As String)
        markerstring = MarketSceName
        ...
        Dt = ObjSceDetail.Select_Scenario()

        For Each dr As DataRow In Dt.Rows

            Dim TAB = New TabPage
            TAB.Text = dr("ScenarioName").ToString
            TAB.Name = dr("ScenarioName").ToString
            TAB.Tag = dr("ScenarioName").ToString
            TAB.ToolTipText = dr("ScenarioName").ToString
            ObjSceDetail.interval = Val(dr("interval") & "")
            ObjSceDetail.interval_type = dr("interval_type").ToString
            If ObjSceDetail.interval_type = "PER" Then
                chkint.Checked = True
            Else
...
Add:
                    ...
                    'ObjSceDetail.CMP = Val(dr("CMP") & "")
                    'ObjSceDetail.CVol = Val(dr("CVol") & "")
                    'ObjSceDetail.EndDate = CDate(dr("EndDate"))
                    'ObjSceDetail.interval = Val(dr("interval") & "")
                    'ObjSceDetail.interval_type = dr("interval_type").ToString
                    'ObjSceDetail.MStrike = Val(dr("MStrike") & "")
                    'ObjSceDetail.PVol = Val(dr("PVol") & "")
                    'ObjSceDetail.ScenarioName = dr("ScenarioName")
                    'ObjSceDetail.SelectedDate = dr("SelectedDate").ToString
                    'ObjSceDetail.StartDate = CDate(dr("StartDate"))
                    'ObjSceDetail.strike = Val(dr("strike") & "")
...
Add2:
            ...
            LoadTag(ScenarioName)
        Else

            'ObjSceDetail.ScenarioName = ScenarioName
            'ObjSceDetail.Delete_scenario_Detail()
            If MarketSceName <> "" Then


                'tbcomp.SelectedTab = tbcomp.TabPages(MarketSceName)
                'ObjSceDetail.ScenarioName = MarketSceName
                'LoadTag(MarketSceName)


                ScenarioName = MarketSceName
                ObjSceDetail.ScenarioName = MarketSceName
                ObjSceDetail.Delete_scenario_Detail()

                If tbcomp.TabPages.Count > 0 Then
                    Dim TAB = New TabPage
                    TAB.Text = MarketSceName
                    TAB.Name = MarketSceName
                    TAB.Tag = MarketSceName
                    TAB.ToolTipText = MarketSceName
                    tbcomp.SelectedTab = tbcomp.TabPages(MarketSceName)
                    If tbcomp.SelectedTab Is Nothing Then
                        GoTo Add3
                    End If
                    If tbcomp.SelectedTab.Name = MarketSceName Then
                        'tbcomp.SelectedTab = tbcomp.TabPages(cnstAnalysisScenario)
                    Else
Add3:
                        tbcomp.TabPages.Add(TAB)
            ...
            LoadTag(MarketSceName)
            Try
                ScenarioName = tbcomp.SelectedTab.Name
            ...
            If (ScenarioName Is Nothing Or ScenarioName = "") Or (ScenarioName = cnstAnalysisScenario) Then
                ...
                '                    GoTo Add2
                '                End If
                If allcompany.allcompanyflag = False Then
                    AddUserDefScenario("")
                End If

            End If
            If ScenarioName Is Nothing Or ScenarioName = "" Then
                Exit Sub

            Else
                If sceallcompanyflag = True Then
                    Dim str As String
                    Dim str1 As String = "NIFTY"
                    If tbcomp.TabPages.ContainsKey(str1) Then
                        tbcomp.SelectedTab = tbcomp.TabPages(str1)
                        ObjSceDetail.ScenarioName = str1
                        LoadTag(str1)
                    Else
                        For Each tab As TabPage In tbcomp.TabPages
                            str = tab.Name
                            If str <> ScenarioName Then
                                tbcomp.SelectedTab = tbcomp.TabPages(str)
                                ObjSceDetail.ScenarioName = str
                                LoadTag(str)
                    ...
                    Call tbcomp_SelectedIndexChanged(Nothing, Nothing)
        ...
        If sceallcompanyflag = True Then
            ...
            If tbcomp.TabPages.ContainsKey(cnstAnalysisScenario) Then
                tbcomp.TabPages.RemoveByKey(cnstAnalysisScenario)
            End If
        End If
...
#Region "Functions"
    Private Function cal_profitLoss() As Double
        ...
        For Each grow As DataGridViewRow In grdtrad.Rows
            If IsDBNull(grow.Cells("Active").Value) Then grow.Cells("Active").Value = False
        ...
        Try
            If fpath <> "" Then
                ...
                For Each drow In tempdata.Rows
                    ...
                    grdtrad.Rows(i).Cells("spval").Value = Val(drow("Underlying"))
                    grdtrad.Rows(i).Cells("Strike").Value = Val(drow("Strike"))
                    grdtrad.Rows(i).Cells("units").Value = Val(drow("Qty"))
                    grdtrad.Rows(i).Cells("ltp").Value = Val(drow("ltp"))
                    grdtrad.Rows(i).Cells("last").Value = Val(drow("Rate"))
    ...
    Private Sub ChkVol()
        ...
        For Each grow As DataGridViewRow In grdtrad.Rows
            If CBool(grow.Cells("Active").Value) = True And Val(grow.Cells("units").Value) <> 0 And grow.Cells("CPF").Value <> "F" And grow.Cells("CPF").Value <> "E" Then
        ...
        End If
    End Sub
    Private Sub ClearallChart()

        'dtprofit.Rows.Clear()
        'dtdeltatable.Rows.Clear()
        'dtgammatable.Rows.Clear()
        'dtvegatable.Rows.Clear()
        'dtthetatable.Rows.Clear()
        'dtvolgatable.Rows.Clear()
        'dtvannatable.Rows.Clear()
        ...
        'chtprofit.ChartData = a
        'chtdelta.ChartData = a
        'chtgamma.ChartData = a
        'chtvega.ChartData = a
        'chttheta.ChartData = a
        'chtvolga.ChartData = a
        'chtvanna.ChartData = a
    End Sub
    ...
    Private Sub result(ByVal ChangeFlg As Boolean)
        ...
        '    Exit Sub
        'End If
        Write_ErrorLogScenario("result Start..")
        div = 1
        str = ""
        If tbcomp.TabPages.Count = 0 Then
            Exit Sub
        ...
        End If
        SaveTag(tbcomp.SelectedTab.Name)
        ...
        If chkint.Checked Then
            If (txtmid - (Val((Val(txtinterval.Text) * txtmid / 100) * Val(txtllimit.Text)))) <= 0 Then Exit Sub
        Else
            If (txtmid - (Val(Val(txtinterval.Text) * Val(txtllimit.Text)))) <= 0 Then Exit Sub
        End If
        ...
        If Val(txtllimit.Text > 0) And Val(interval) > 0 And Val(txtmid) > 0 Then
            ...
            'grdtheta.Refresh()
        End If
        Write_ErrorLogScenario("result Stop..")
    End Sub
...
#Region "Calculation"
    ...
    Private Sub CalData(ByVal futval As Double, ByVal stkprice As Double, ByVal cpprice As Double, ByVal mT As Double, ByVal mmT As Double, ByVal mIsCall As Boolean, ByVal mIsFut As Boolean, ByVal grow As DataGridViewRow, ByVal qty As Double, ByVal lv As Double, ByVal DifFactor As Double)
        ...
        If INCLUDEEXPIRY_VOLANDGREEK_CAL = 1 And mT <> 0 Then
            _mt = ((mT) / 365) + 1
            _mt1 = ((mmT)) / 365 + 1
        ...
        Try
            ...
            grow.Cells("gamma1").Value = Math.Round(mGama1, DecimalSetting.iRGamma)
            grow.Cells("gmval1").Value = Math.Round(mGama1 * qty, DecimalSetting.iRGammaval)
            grow.Cells("vega1").Value = Math.Round(mVega1, DecimalSetting.iRVega)
            grow.Cells("vgval1").Value = Math.Round(mVega1 * qty, DecimalSetting.iRVegaval)
            grow.Cells("theta1").Value = Math.Round(mThita1, DecimalSetting.iRTheta)
    ...
    End Sub
    Private Sub SetDecDate()

        dtprofit = profit.Copy
        dtdeltatable = deltatable.Copy
        dtgammatable = gammatable.Copy
        dtvegatable = vegatable.Copy
        dtthetatable = thetatable.Copy
        dtvolgatable = volgatable.Copy
        dtvannatable = vannatable.Copy

        profitarr = New ArrayList
        deltaarr = New ArrayList
        gammaarr = New ArrayList
        vegaarr = New ArrayList
        ...
        Try
            For Each dr As DataRow In dtprofit.Rows
                For Each col As DataColumn In dtprofit.Columns
                    If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                        dr(col.ColumnName) = dr(col.ColumnName) / div
                        profitarr.Add(dr(col.ColumnName))
                    End If
                Next
            Next
            dtprofit.AcceptChanges()
            grdprofit.DataSource = dtprofit
            objProfitLossChart.grdprofit.DataSource = dtprofit
            create_chart_profit()
            'create_chart_profit_Full()

            For Each dr As DataRow In dtdeltatable.Rows
                For Each col As DataColumn In dtdeltatable.Columns
                    If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                        dr(col.ColumnName) = dr(col.ColumnName) / div
                ...
                Next
            Next
            dtdeltatable.AcceptChanges()
            grddelta.DataSource = dtdeltatable
            objProfitLossChart.grddelta.DataSource = dtdeltatable
            create_chart_delta()
            'create_chart_delta_Full()

            For Each dr As DataRow In dtgammatable.Rows
                For Each col As DataColumn In dtgammatable.Columns
                    If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                        dr(col.ColumnName) = dr(col.ColumnName) / div
                ...
                Next
            Next
            dtgammatable.AcceptChanges()
            grdgamma.DataSource = dtgammatable
            objProfitLossChart.grdgamma.DataSource = dtgammatable
            create_chart_gamma()
            ' create_chart_gamma_Full()

            For Each dr As DataRow In dtvegatable.Rows
                For Each col As DataColumn In dtvegatable.Columns
                    If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                        dr(col.ColumnName) = dr(col.ColumnName) / div
                ...
                Next
            Next
            dtvegatable.AcceptChanges()
            grdvega.DataSource = dtvegatable
            objProfitLossChart.grdvega.DataSource = dtvegatable
            create_chart_vega()
            'create_chart_vega_Full()


            For Each dr As DataRow In dtthetatable.Rows
                For Each col As DataColumn In dtthetatable.Columns
                    If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                        dr(col.ColumnName) = dr(col.ColumnName) / div
                ...
                Next
            Next
            dtthetatable.AcceptChanges()
            grdtheta.DataSource = dtthetatable
            objProfitLossChart.grdtheta.DataSource = dtthetatable
            create_chart_theta()
            '  create_chart_theta_Full()


            For Each dr As DataRow In dtvolgatable.Rows
                For Each col As DataColumn In dtvolgatable.Columns
                    If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                        dr(col.ColumnName) = dr(col.ColumnName) / div
                ...
                Next
            Next
            dtvolgatable.AcceptChanges()
            grdvolga.DataSource = dtvolgatable
            objProfitLossChart.grdvolga.DataSource = dtvolgatable
            create_chart_volga()
            'create_chart_volga_Full()

            For Each dr As DataRow In dtvannatable.Rows
                For Each col As DataColumn In dtvannatable.Columns
                    If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                        dr(col.ColumnName) = dr(col.ColumnName) / div
                ...
                Next
            Next
            dtvannatable.AcceptChanges()
            grdvanna.DataSource = dtvannatable
            objProfitLossChart.grdvanna.DataSource = dtvannatable
            create_chart_vanna()
            'create_chart_vanna_Full()
    ...
    Private Function cal_profit() As Boolean
        grdprofit.DataSource = Nothing
        objProfitLossChart.grdprofit.DataSource = Nothing
        'grdprofit.Refresh()
                ...
                If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                    For Each grow As DataGridViewRow In grdtrad.Rows
                        ...
                        If CBool(grow.Cells("Active").Value) = True And Val(grow.Cells("units").Value) <> 0 Then
                            Dim SpotValue As Double = Val(dr("SpotValue")) + Val(grow.Cells("DifFactor").Value)
                            If grow.Cells("CPF").Value = "F" Or grow.Cells("CPF").Value = "E" Then
                                ...
                                End If
                                dr(col.ColumnName) = (Val(dr(col.ColumnName)) + ((Val(SpotValue) - Val(grow.Cells("ltp").Value)) * Val(grow.Cells("units").Value)))
                            ...
                            Else
                                ...
                                If CDate(cd.Date) = CDate(grow.Cells("TimeII").Value).Date Then
                                    If (IsDate(col.ColumnName)) Then
                                        If CDate(dtexp.Value.Date) > CDate(grow.Cells("TimeII").Value).Date Then
                                            _mT = 0
                                        Else
                                            _mT = 0.5
                                        End If
                                ...
                                '    _mT = Get_DayTime_mt(CDate(dtexp.Value.Date), _mT1)

                                'End If


                                If dr("Percent(%)") = "10" Then
                                    Dim abc As Int16 = 0
                                End If
                                ...
                                If (IsDate(col.ColumnName)) Then
                                    ...
                                    bval = Greeks.Black_Scholes(SpotValue, Val(grow.Cells("Strike").Value), Mrateofinterast, 0, (Val(grow.Cells("lv").Value)) / 100, _mT, iscall, True, 0, 0)
                                    totcpr = ((Val(bval & "") - Val(grow.Cells("ltp").Value)) * Val(grow.Cells("units").Value))
                                ...
                                Else
                                    If (iscall = True) Then
                                        If ((SpotValue - Val(grow.Cells("Strike").Value)) > 0) Then
                                            bval = (SpotValue - Val(grow.Cells("Strike").Value))
                                            totcpr = ((bval - Val(grow.Cells("ltp").Value)) * Val(grow.Cells("units").Value))
                                            dr(col.ColumnName) = (Val(dr(col.ColumnName)) + totcpr)
                                        Else
                                        ...
                                        End If
                                    Else
                                        If ((Val(grow.Cells("Strike").Value) - SpotValue) > 0) Then
                                            bval = (Val(grow.Cells("Strike").Value) - SpotValue)
                                            totcpr = ((bval - Val(grow.Cells("ltp").Value)) * Val(grow.Cells("units").Value))
                                            dr(col.ColumnName) = (Val(dr(col.ColumnName)) + totcpr)
                                        Else
                                            bval = 0 'CDbl(dr(col.ColumnName)) + ((0 - CDbl(grow.Cells(7).Value)) * CDbl(grow.Cells(6).Value))
                        ...
                        End If
                        Debug.WriteLine(dr("Percent(%)") & " : " & dr(col.ColumnName))
                    Next
                    dr(col.ColumnName) += grossmtm
                    dcount = dcount + 1
                    Debug.WriteLine(dr("Percent(%)"))
                End If
                i = i + 1


                'profitarr.Add(dr(col.ColumnName) / div)
                profitarr.Add(dr(col.ColumnName))
        ...
        'SetDecDate(profit)
        grdprofit.DataSource = profit
        objProfitLossChart.grdprofit.DataSource = profit


        Try
            grdprofit.ClearSelection()
            objProfitLossChart.grdprofit.ClearSelection()
            ...
            For Each grow As DataGridViewRow In objProfitLossChart.grdprofit.Rows
                If grow.Cells(1).Value = 0 Then
                    grow.Selected = True
                    objProfitLossChart.grdprofit.FirstDisplayedScrollingRowIndex = (Val(txtllimit.Text) / 2) 'grow.Index
                    Exit For
                Else
                    'grow.Selected = False
                End If
    ...
    Private Function cal_delta() As Boolean
        ...
        grddelta.DataSource = Nothing
        objProfitLossChart.grddelta.DataSource = Nothing
        'grddelta.Refresh()
        Dim totfpr As Double
        Dim totcpr As Double
        Dim bval As Double
        ...
        For Each dr As DataRow In deltatable.Rows
            ...
            For Each col As DataColumn In deltatable.Columns
                ...
                End If
                i = i + 1
                'deltaarr.Add(dr(col.ColumnName) / div)
                deltaarr.Add(dr(col.ColumnName))
            Next
        Next
        'SetDecDate(deltatable)
        grddelta.DataSource = deltatable
        objProfitLossChart.grddelta.DataSource = deltatable

        Try
            grddelta.ClearSelection()
            objProfitLossChart.grddelta.ClearSelection()
            For Each grow As DataGridViewRow In grddelta.Rows
                ...
                End If
            Next
            For Each grow As DataGridViewRow In objProfitLossChart.grddelta.Rows
                If grow.Cells(1).Value = 0 Then
                    grow.Selected = True
                    objProfitLossChart.grddelta.FirstDisplayedScrollingRowIndex = (Val(txtllimit.Text) / 2) 'grow.Index
                    Exit For
                Else
                    'grow.Selected = False
                End If
    ...
    Private Function cal_gamma() As Boolean
        grdgamma.DataSource = Nothing
        objProfitLossChart.grdgamma.DataSource = Nothing
        'grdgamma.Refresh()
        Dim totfpr As Double
        Dim totcpr As Double
        ...
        For Each dr As DataRow In gammatable.Rows
            ...
            For Each col As DataColumn In gammatable.Columns
                ...
                End If
                i = i + 1
                'gammaarr.Add(dr(col.ColumnName) / div)
                gammaarr.Add(dr(col.ColumnName))
            Next
        Next
        'SetDecDate(gammatable)
        grdgamma.DataSource = gammatable
        objProfitLossChart.grdgamma.DataSource = gammatable

        Try
            grdgamma.ClearSelection()
            objProfitLossChart.grdgamma.ClearSelection()
            ...
            For Each grow As DataGridViewRow In objProfitLossChart.grdgamma.Rows
                If grow.Cells(1).Value = 0 Then
                    grow.Selected = True
                    objProfitLossChart.grdgamma.FirstDisplayedScrollingRowIndex = (Val(txtllimit.Text) / 2) 'grow.Index
                    Exit For
                Else
                    'grow.Selected = False
                End If
    ...
    Private Function cal_vega() As Boolean
        ...
        grdvega.DataSource = Nothing
        objProfitLossChart.grdvega.DataSource = Nothing
        'grdvega.Refresh()
        Dim totfpr As Double
        Dim totcpr As Double
        Dim bval As Double
        ...
        For Each dr As DataRow In vegatable.Rows
            ...
            For Each col As DataColumn In vegatable.Columns
                ...
                End If
                i = i + 1
                'vegaarr.Add(dr(col.ColumnName) / div)
                vegaarr.Add(dr(col.ColumnName))
            Next
        Next

        'SetDecDate(vegatable)
        grdvega.DataSource = vegatable
        objProfitLossChart.grdvega.DataSource = vegatable
        Try
            grdvega.ClearSelection()
            objProfitLossChart.grdvega.ClearSelection()
            For Each grow As DataGridViewRow In grdvega.Rows
                ...
                End If
            Next
            For Each grow As DataGridViewRow In objProfitLossChart.grdvega.Rows
                If grow.Cells(1).Value = 0 Then
                    grow.Selected = True
                    objProfitLossChart.grdvega.FirstDisplayedScrollingRowIndex = (Val(txtllimit.Text) / 2) 'grow.Index
                    Exit For
                Else
                    'grow.Selected = False
                End If
    ...
    Private Function cal_theta() As Boolean
        grdtheta.DataSource = Nothing
        objProfitLossChart.grdtheta.DataSource = Nothing
        'grdtheta.Refresh()
        Dim totfpr As Double
        Dim totcpr As Double
        ...
        For Each dr As DataRow In thetatable.Rows
            ...
            For Each col As DataColumn In thetatable.Columns
                ...
                End If
                i = i + 1
                'thetaarr.Add(dr(col.ColumnName) / div)
                thetaarr.Add(dr(col.ColumnName))
            Next
        Next
        'SetDecDate(thetatable)

        grdtheta.DataSource = thetatable
        objProfitLossChart.grdtheta.DataSource = thetatable
        Try
            grdtheta.ClearSelection()
            objProfitLossChart.grdtheta.ClearSelection()
            ...
            For Each grow As DataGridViewRow In objProfitLossChart.grdtheta.Rows
                If grow.Cells(1).Value = 0 Then
                    grow.Selected = True
                    objProfitLossChart.grdtheta.FirstDisplayedScrollingRowIndex = (Val(txtllimit.Text) / 2) 'grow.Index
                    Exit For
                Else
                    'grow.Selected = False
                End If
    ...
    Private Function cal_volga() As Boolean
        grdvolga.DataSource = Nothing
        objProfitLossChart.grdvolga.DataSource = Nothing
        'grdvolga.Refresh()
        Dim totfpr As Double
        Dim totcpr As Double
        ...
        For Each dr As DataRow In volgatable.Rows
            ...
            For Each col As DataColumn In volgatable.Columns
                ...
                End If
                i = i + 1
                'volgaarr.Add(dr(col.ColumnName) / div)
                volgaarr.Add(dr(col.ColumnName))
                'If volgaarr(3) = "NaN" Then MsgBox("A")
            Next
        Next

        'SetDecDate(volgatable)
        grdvolga.DataSource = volgatable
        objProfitLossChart.grdvolga.DataSource = volgatable
        Try
            grdvolga.ClearSelection()
            objProfitLossChart.grdvolga.ClearSelection()
            ...
            For Each grow As DataGridViewRow In objProfitLossChart.grdvolga.Rows
                If grow.Cells(1).Value = 0 Then
                    grow.Selected = True
                    objProfitLossChart.grdvolga.FirstDisplayedScrollingRowIndex = (Val(txtllimit.Text) / 2) 'grow.Index
                    Exit For
                Else
                    'grow.Selected = False
                End If
    ...
    Private Function cal_vanna() As Boolean
        grdvanna.DataSource = Nothing
        objProfitLossChart.grdvanna.DataSource = Nothing
        'grdvanna.Refresh()
        Dim totfpr As Double
        Dim totcpr As Double
        ...
        For Each dr As DataRow In vannatable.Rows
            ...
            For Each col As DataColumn In vannatable.Columns
                ...
                End If
                i = i + 1
                'vannaarr.Add(dr(col.ColumnName) / div)
                vannaarr.Add(dr(col.ColumnName))
            Next
        Next

        'SetDecDate(vannatable)
        grdvanna.DataSource = vannatable
        objProfitLossChart.grdvanna.DataSource = vannatable

        Try
            grdvanna.ClearSelection()
            objProfitLossChart.grdvanna.ClearSelection()
            For Each grow As DataGridViewRow In grdvanna.Rows
                ...
                End If
            Next
            For Each grow As DataGridViewRow In objProfitLossChart.grdvanna.Rows
                If grow.Cells(1).Value = 0 Then
                    grow.Selected = True
                    objProfitLossChart.grdvanna.FirstDisplayedScrollingRowIndex = (Val(txtllimit.Text) / 2) 'grow.Index
                    Exit For
                Else
                    'grow.Selected = False
                End If
    ...
    End Function
    '    Private Sub create_activeold(Optional ByVal WithOutVol As Boolean = False)
    ...
    '        Dim tdate As Date
    '        Dim STP As Integer
    '        If (i / Val(TxtNoDate.Text)) < 1 Then
    '            STP = 1
    '        Else
    '            STP = (i / Val(TxtNoDate.Text))
    '        End If
    '        Dim Intvar As Integer = 0
    'next0:
    '        Dim tdate1 As String
    '        tdate = Format(DateAdd(DateInterval.Day, Intvar, dttoday.Value), "MM/dd/yyyy")
    '        tdate1 = tdate.ToString("MM/dd/yyyy")
    '        If CDate(tdate) > CDate(dtexp.Value.Date) Then
    '            GoTo next1
    '        End If
    '        If UCase(WeekdayName(Weekday(tdate))) = "SATURDAY" Then
    '            Intvar = Intvar + 2
    '            tdate = Format(DateAdd(DateInterval.Day, Intvar, dttoday.Value), "MM/dd/yyyy")
    '        End If
    '        Intvar = Intvar + 1
    ...
    '        Try
    '            temptable.Columns.Add(tdate1, GetType(Boolean))
    '        Catch ex As Exception
    ...
    '        gcol.DataPropertyName = tdate1
    '        gcol.Name = tdate1
    '        gcol.Width = 70
    '        gcol.DisplayIndex = gcol.Index
    ...
    '        End Try
    '        gcol = New DataGridViewCheckBoxColumn
    '        gcol.HeaderText = tdate.grdact.Columns.Add("dd-MMM")
    '        gcol.DataPropertyName = tdate1
    '        gcol.Name = tdate1
    '        gcol.Width = 70
    '        gcol.DisplayIndex = gcol.Index
    ...
    '        For k As Integer = (Intvar + 1) To i - 1 Step STP
    ...
    '        gcol.DataPropertyName = "Expiry " & tdate1
    '        gcol.Name = "Expiry " & tdate1
    '        gcol.DisplayIndex = gcol.Index
    '        grdact.Columns.Add(gcol)
    '        temptable.Columns.Add("Expiry " & tdate1, GetType(Boolean))
    '        gcol.DefaultCellStyle.NullValue = False
    '        'End If
    ...
    '            tdate = Format(DateAdd(DateInterval.Day, 0, dttoday.Value), "MM/dd/yyyy")
    '            tdate1 = tdate.ToString("MM/dd/yyyy")
    '            Try


    '                If (DateDiff(DateInterval.Day, dttoday.Value.Date, dtexp.Value.Date)) <> 0 Then
    '                    If UCase(WeekdayName(Weekday(tdate1))) <> "SUNDAY" And UCase(WeekdayName(Weekday(tdate1))) <> "SATURDAY" Then
    '                        drow(tdate1) = True
    '                    Else
    '                        drow(tdate1) = False
    '                    End If
    ...
    '            Try
    '                tdate = Format(DateAdd(DateInterval.Day, (i - 1), dttoday.Value), "MM/dd/yyyy")
    '                tdate1 = tdate.ToString("MM/dd/yyyy")
    '                If UCase(WeekdayName(Weekday(tdate))) <> "SUNDAY" And UCase(WeekdayName(Weekday(tdate))) <> "SATURDAY" Then
    '                    'drow(tdate) = True
    '                    drow("Expiry " & tdate1) = True
    '                Else
    '                    drow(tdate) = False
    '                    drow("Expiry " & tdate1) = False
    '                End If
    '            Catch ex As Exception
    ...
    '            bCheckAll = True
    '            tdate = Format(dtexp.Value, "MM/dd/yyyy")
    '            tdate1 = tdate.ToString("MM/dd/yyyy")
    ...
    Private Sub create_active(Optional ByVal WithOutVol As Boolean = False)
        ...
        'tdate = Format(DateAdd(DateInterval.Day, Intvar, dttoday.Value), "dd/MMM/yyyy")
        'tdate2 = tdate.ToString("dd/MMM/yyyy")

        'If tdate.ToString("dd/MMM/yyyy") = "22/Oct/2016" Then
        '    tdate2 = tdate.ToString("dd/MMM/yyyy")
        'End If
...
next0:
        Dim tdate1 As String
        tdate = Format(DateAdd(DateInterval.Day, Intvar, dttoday.Value), "dd/MMM/yyyy")
        tdate1 = tdate.ToString("dd/MMM/yyyy")
        If CDate(tdate) > CDate(dtexp.Value.Date) Then
            GoTo next1
        End If

        '    Dim tdate As Date = DateFormat(gcol.DataPropertyName)
        '    Dim tdate1 As String = tdate.ToString("dd/MMM/yyyy")
        '    Dim oldDate As Date = DateSerial(tdate.Year, tdate.Month, tdate.Day)


        '    Dim oldWeekDayName As String = oldDate.DayOfWeek.ToString()
        '    If UCase(oldWeekDayName) = "SUNDAY" Or UCase(oldWeekDayName) = "SATURDAY" Then
        '        Continue For
        '    End If
        'If UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) = "SATURDAY" Then
        ...
        Try
            ...
            'If UCase(WeekdayName(Weekday(tdate))) = "SUNDAY" Or UCase(WeekdayName(Weekday(tdate))) = "SATURDAY" Then
            If UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) = "SUNDAY" Or UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) = "SATURDAY" Then
            Else
                temptable.Columns.Add(tdate1, GetType(Boolean))
            End If
        ...
        'For k As Integer = (Intvar + 1) To i - 1 Step 1

        '    tdate = Format(DateAdd(DateInterval.Day, k, dttoday.Value), "dd/MMM/yyyy")
        '    tdate1 = tdate.ToString("dd/MMM/yyyy")
        '    'If UCase(WeekdayName(Weekday(tdate))) <> "SUNDAY" And UCase(WeekdayName(Weekday(tdate))) <> "SATURDAY" Then

        '    If UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) = "THURSDAY" Then
        '        Try
        '            thr_Hash.Add(tdate1, CDate(tdate1))
        '        Catch ex As Exception
...
next3:
        ...
        For k As Integer = (Intvar + 1) To i - 1 Step STP
            ...
            'If UCase(WeekdayName(Weekday(tdate))) <> "SUNDAY" And UCase(WeekdayName(Weekday(tdate))) <> "SATURDAY" Then
            If UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) = "SUNDAY" Or UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) = "SATURDAY" Then
            ElseIf UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) = "thursday" Then
                Try
                    temptable.Columns.Add(tdate1, GetType(Boolean))
                Catch ex As Exception
                    If ex.Message.Contains("already belongs to this DataTable") Then
        ...
        If grdact.Columns.Count > 0 Then
            ...
            Try
                ...
                If (DateDiff(DateInterval.Day, dttoday.Value.Date, dtexp.Value.Date)) <> 0 Then
                    If UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) <> "SUNDAY" And UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) <> "SATURDAY" Then
                        drow("tdate") = True
                    Else
                        drow("tdate") = False
                    End If
            ...
            Dim i1 As Integer = 0
            If CDate(dttoday.Value.Date) < CDate(dtexp.Value.Date) Then
                i1 = (DateDiff(DateInterval.Day, dttoday.Value.Date, dtexp.Value.Date))
            End If
            Intvar = 0
            For k1 As Integer = (Intvar + 1) To i1 - 1 Step 1

                tdate = Format(DateAdd(DateInterval.Day, k1, dttoday.Value), "dd/MMM/yyyy")
                tdate1 = tdate.ToString("dd/MMM/yyyy")


                If UCase(DateSerial(tdate.Year, tdate.Month, tdate.Day).DayOfWeek.ToString()) = "THURSDAY" Then
                    Try
                        If temptable.Columns.Contains(tdate1) = False Then
                            temptable.Columns.Add(tdate1, GetType(Boolean))
                            gcol = New DataGridViewCheckBoxColumn
                            gcol.HeaderText = tdate.ToString("dd-MMM")
                            gcol.DataPropertyName = tdate1
            ...
            Dim tempcopy As New DataTable
            tempcopy.Columns.Add("Ddate", GetType(Date))

            Dim dr2 As DataRow
            For Each dt As DataColumn In temptable.Columns
                Try

                    Try
                        dr2 = tempcopy.NewRow
                        dr2("Ddate") = CDate(dt.ColumnName)
                        tempcopy.Rows.Add(dr2)
                    Catch ex As Exception
                        'dr2("Ddate") = dt.ColumnName
                        'tempcopy.Rows.Add(dr2)
                        'Continue For
                    End Try
            ...
            Dim dataView As New DataView(tempcopy)
            dataView.Sort = "Ddate"
            Dim dataTable As DataTable = dataView.ToTable()
            'temptable = New DataTable

            For Each dt As DataRow In dataTable.Rows
                If (datadt.Columns.Contains(CDate(dt("Ddate")).ToString("dd-MMM")) = False) Then

                    datadt.Columns.Add(CDate(dt("Ddate")).ToString("dd-MMM"), GetType(Date))
                End If
            Next

            'Try
            '    For i22 As Integer = 0 To datadt.Columns.Count - 1
            '        If temptable.Columns.Contains(datadt.Columns(i22).ColumnName) Then
            '            'temptable.Columns(dataTable.Columns(i).ColumnName.ToString()).SetOrdinal = i
            '            temptable.Columns(datadt.Columns(i22).ColumnName.ToString).SetOrdinal(i22)
            '        End If
            '    Next
            'Catch ex As Exception
            ...
            'For Each dt As DataRow In dataTable.Rows
            '    'temptable.Columns.Add(tdate1, GetType(Boolean))
            '    tdate1 = CDate(dt("Ddate")).ToString("dd-MMM")
            '    gcol = New DataGridViewCheckBoxColumn
            '    gcol.HeaderText = tdate1
            '    gcol.DataPropertyName = tdate1
            '    gcol.Name = tdate1
            '    gcol.Width = 70
            '    gcol.DisplayIndex = gcol.Index
            '    grdact.Columns.Add(gcol)
            '    If temptable.Columns.Contains(tdate) = False Then
            '        temptable.Columns.Add(tdate, GetType(Boolean))
            '    End If
            ...
            For i33 As Integer = 0 To datadt.Columns.Count - 1
                For Each gcol1 As DataGridViewCheckBoxColumn In grdact.Columns

                    Dim c As String = gcol1.HeaderText
                    If c = datadt.Columns(i33).ColumnName.ToString Then
                        gcol1.DisplayIndex = i33
                    End If
            ...
            Try
                'Dim jj As Integer = temptable.Columns.Count
                'temptable.Columns.RemoveAt(jj)
                'grdact.Columns.RemoveAt(jj)
                'grdact.Refresh()
    ...
    '        If UCase(oldWeekDayName) = "SATURDAY" Then
    '            Intvar = Intvar + 2
    '            tdate = Format(DateAdd(DateInterval.Day, Intvar, dttoday.Value), "dd/MMM/yyyy")
    '            tdate1 = tdate.ToString("dd/MMM/yyyy")
    '        End If
    ...
    '        'If UCase(WeekdayName(Weekday(tdate))) <> "SUNDAY" And UCase(WeekdayName(Weekday(tdate))) <> "SATURDAY" Then
    '        Try
    '            oldDate = DateSerial(tdate.Year, tdate.Month, tdate.Day)
    ...
    '        gcol = New DataGridViewCheckBoxColumn
    '        gcol.HeaderText = tdate.ToString("dd-MMM")
    '        gcol.DataPropertyName = tdate.ToString("MM/dd/yyyy")
    '        gcol.Name = tdate.ToString("MM/dd/yyyy")
    '        gcol.Width = 70
    '        gcol.DisplayIndex = gcol.Index
    '        grdact.Columns.Add(gcol)
    '        'temptable.Columns.Add(tdate, GetType(Boolean))
    ...
    '        End If
    '    End Sub
    Private Sub create_act(ByVal dt As DataTable)
        ' '' ''grdact.Columns.Clear()
        ...
        ' '' ''            drow(Format(dtr("date"), "MM/dd/yyyy")) = True
        ' '' ''        Next
        ' '' ''    Catch ex As Exception
        ...
        Try
            ...
            Dim temptable As New DataTable
            temptable = CType(grdact.DataSource, DataTable)
            ...
            f2 = 1
            For Each dtr As DataRow In dt.Rows
                tdate = CDate(dtr("Date")).ToString("dd/MMM/yyyy")

                For Each dcol As DataColumn In temptable.Columns


                    If dcol.ColumnName.Contains(CDate(dtr("Date")).ToString("dd/MMM/yyyy")) Then
                        f1 = 1
                ...
                ElseIf f1 = 2 Then
                    ...
                    Dim tdate2 As String
                    tdate2 = tdate.ToString("dd/MMM/yyyy")
                    temptable.Columns.Add(tdate2, GetType(Boolean))
                    temptable.Rows(0)(tdate2) = True
                    ...
                    If temp1.Columns.Contains("Ddate") Then
                    Else
                        temp1.Columns.Add("Ddate", GetType(Date))
                    End If



                    Dim DROW1 As DataRow
                    For Each dcol1 As DataColumn In temptable.Columns
                        DROW1 = temp1.NewRow
                        If IsDate(dcol1.ColumnName) Then
                            DROW1("Ddate") = CDate(dcol1.ColumnName)
                            temp1.Rows.Add(DROW1)
                        End If
                    Next
                    Dim dataView As New DataView(temp1)
                    dataView.Sort = "Ddate ASC"
                    Dim dataTable As DataTable = dataView.ToTable()
                    '    Dim i1 As Integer = 0
                    grdact.Columns.Clear()

                    For Each row1 As DataRow In dataTable.Select("", "Ddate")

                        gcol = New DataGridViewCheckBoxColumn
                        gcol.HeaderText = CDate(row1("Ddate")).ToString("dd-MMM")
                        gcol.DataPropertyName = CDate(row1("Ddate")).ToString("dd/MMM/yyyy")
                        gcol.Name = CDate(row1("Ddate")).ToString("dd/MMM/yyyy")
                        gcol.Width = 70
                        gcol.DisplayIndex = i1
                        gcol.DefaultCellStyle.NullValue = False
                        grdact.Columns.Add(gcol)
                        i1 = i1 + 1
                    Next
            ...
            Dim TDATE3 As String
            Dim isColFind As Boolean
            temptable = CType(grdact.DataSource, DataTable)

            '    Dim i1 As Integer = 0
            For Each dcol As DataRow In temp1.Rows
                tdate = dcol("Ddate")
                TDATE3 = tdate.ToString("dd/MMM/yyyy")
                For Each dtr As DataRow In dt.Rows

                    If dcol("Ddate") = dtr("date") Then
                        isColFind = True
                        Exit For
                    Else
                        isColFind = False
                ...
                If isColFind = True Then
                    ' temptable.Rows(0)(CDate(dtr("date")).ToString("MM/dd/yyyy")) = True
                    temptable.Rows(0)(TDATE3) = True

                ElseIf isColFind = False Then

                    temptable.Rows(0)(TDATE3) = False
                    '   temptable.Rows(0)(CDate(dtr("date")).ToString("MM/dd/yyyy")) = True
            ...
            Dim dr2 As DataRow
            For Each dt1 As DataColumn In temptable.Columns
                Try

                    Try
                        dr2 = tempcopy.NewRow
                        dr2("Ddate") = CDate(dt1.ColumnName)
                        tempcopy.Rows.Add(dr2)
                    Catch ex As Exception
                        'dr2("Ddate") = dt.ColumnName
                        'tempcopy.Rows.Add(dr2)
            ...
            Dim dataView11 As New DataView(tempcopy)
            dataView11.Sort = "Ddate"
            Dim dataTable111 As DataTable = dataView11.ToTable()
            'temptable = New DataTable

            For Each dt2 As DataRow In dataTable111.Rows
                If (datadt.Columns.Contains(CDate(dt2("Ddate")).ToString("dd/MMM/yyyy")) = False) Then

                    datadt.Columns.Add(CDate(dt2("Ddate")).ToString("dd/MMM/yyyy"), GetType(Date))
                End If
            Next
            ...
            DtTmp.Columns.Add("Ddate", GetType(Date))
            Dim DROW As DataRow
            For Each dcol As DataColumn In temptable.Columns
                DROW = DtTmp.NewRow
                If IsDate(dcol.ColumnName) Then
                    DROW("Ddate") = CDate(dcol.ColumnName)
                    DtTmp.Rows.Add(DROW)
                End If
            Next
            Dim i As Integer = 0
            ...
            For Each row As DataRow In DtTmp.Select("", "Ddate")
                gcol = New DataGridViewCheckBoxColumn
                gcol.HeaderText = CDate(row("Ddate")).ToString("dd-MMM")
                gcol.DataPropertyName = CDate(row("Ddate")).ToString("dd/MMM/yyyy")
                gcol.Name = CDate(row("Ddate")).ToString("dd/MMM/yyyy")
                gcol.Width = 70
                gcol.DisplayIndex = i
                gcol.DefaultCellStyle.NullValue = False
                grdact.Columns.Add(gcol)
            ...
            gcol = New DataGridViewCheckBoxColumn
            gcol.HeaderText = "Expiry"
            gcol.DataPropertyName = "Expiry " & CDate(tdate1)
            gcol.Name = "Expiry " '& CDate(tdate1)
            gcol.DisplayIndex = gcol.Index
            grdact.Columns.Add(gcol)
            'temptable.Columns.Add("Expiry " & tdate, GetType(Boolean))
            gcol.DefaultCellStyle.NullValue = False
    ...
    '        'Dim drow As DataRow
    '        For Each dtr As DataRow In dt.Rows
    '            Dim isColFind As Boolean = False
    '            For Each dcol As DataColumn In temptable.Columns

    '                'dcol.ColumnName( (Format(dtr("date"), "MM/dd/yyyy")) = True
    '                If dcol.ColumnName.Contains(CDate(dtr("date")).ToString("MM/dd/yyyy")) Then
    '                    isColFind = True
    '                    Exit For
    '                End If
    '            Next
    ...
    '            Dim tdate2 As String
    '            ' tdate1 = tdate.ToString("MM/dd/yyyy")
    '            If isColFind = False Then
    '                tdate = CDate(dtr("date")).ToString("dd/MMM/yyyy")

    '                tdate2 = tdate.ToString("MM/dd/yyyy")
    '                Try
    '                    temptable.Columns.Add(tdate2, GetType(Boolean))
    '                    'gcol = New DataGridViewCheckBoxColumn
    '                    'gcol.HeaderText = tdate.ToString("dd-MMM")
    ...
    '                End Try
    '                temptable.Rows(0)(CDate(dtr("date")).ToString("MM/dd/yyyy")) = True
    '                'grdact.Item(CDate(dtr("date")).ToString("MM/dd/yyyy"), 0).Value = True
    '            Else
    '                grdact.Item(CDate(dtr("date")).ToString("MM/dd/yyyy"), 0).Value = True
    '            End If
    '        Next
    ...
    '            gcol = New DataGridViewCheckBoxColumn
    '            gcol.HeaderText = CDate(row("Ddate")).ToString("dd-MMM")
    '            gcol.DataPropertyName = CDate(row("Ddate")).ToString("MM/dd/yyyy")
    '            gcol.Name = CDate(row("Ddate")).ToString("MM/dd/yyyy")
    '            gcol.Width = 70
    '            gcol.DisplayIndex = i
    '            gcol.DefaultCellStyle.NullValue = False
    '            grdact.Columns.Add(gcol)
    ...
    '            ''gcol = New DataGridViewCheckBoxColumn
    '            'grdact.Columns(CDate(row("Ddate")).ToString("MM/dd/yyyy")).HeaderText = CDate(row("Ddate")).ToString("dd-MMM")
    '            'grdact.Columns(CDate(row("Ddate")).ToString("MM/dd/yyyy")).DataPropertyName = CDate(row("Ddate")).ToString("MM/dd/yyyy")
    '            'grdact.Columns(CDate(row("Ddate")).ToString("MM/dd/yyyy")).Name = CDate(row("Ddate")).ToString("MM/dd/yyyy")
    '            'grdact.Columns(CDate(row("Ddate")).ToString("MM/dd/yyyy")).Width = 70
    '            'grdact.Columns(CDate(row("Ddate")).ToString("MM/dd/yyyy")).DefaultCellStyle.NullValue = False
    '            'grdact.Columns(CDate(row("Ddate")).ToString("MM/dd/yyyy")).DisplayIndex = i
    ...
    Private Sub create_chart_profit()
        ...
        Dim chartData As CChartData = chartDataCol.mChartDataProfitLoss
        Dim chaDbl(profit.Rows.Count - 1, profit.Columns.Count - 3) As Double
        Dim xData(profit.Rows.Count) As Double
        ...
        If profit.Columns.Count > 1 Then
            ...
            For Each col As DataColumn In profit.Columns
                If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                    ...
                    For Each drow As DataRow In profit.Rows
                        ...
                        chaDbl(r, c) = Val(drow(col.ColumnName)) / div
                        r += 1
                    Next
                    c += 1
                End If
            ...
            '    Dim legendLabels(profit.Columns.Count) As String
            chartData.mLegendList.Clear()
            For Each col As DataColumn In profit.Columns
                If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                    '     chtprofit.Column = c
                    '     chtprofit.ColumnLabel = Format(DateAdd(DateInterval.Day, i, dttoday.Value), "dd/MMM/yyyy")
                    chartData.mLegendList.Add(col.ColumnName)
                    i += 1
                    c += 1
                End If
            ...
            Next
            r = 1
            Dim rowLbl As String
            For Each drow As DataRow In profit.Rows
                '  chtprofit.Row = r
                If drow("spotvalue") >= 1000 Then
                    'chtprofit.RowLabel = Int(drow("spotvalue"))
                    rowLbl = Int(drow("spotvalue"))
                    xData(r - 1) = CDbl(rowLbl)
                Else
                    'chtprofit.RowLabel = drow("spotvalue")
                    rowLbl = drow("spotvalue")
                    xData(r - 1) = CDbl(rowLbl)
                End If
                r += 1
            Next
        ...
        chartData.mYDataLines = chaDbl
        chartData.mXData = xData
        chartData.mFormPlot.Plot.Remove(Of Scatter)(Function(s) True)
        chartDataCol.SetPlotData(chartData)

        Dim chartDatafull As CChartData = objProfitLossChart.chartDataCol.mChartDataProfitLoss
        chartDatafull.mLegendList = chartData.mLegendList
        chartDatafull.mYDataLines = chaDbl
        chartDatafull.mXData = xData
        chartDatafull.mFormPlot.Plot.Remove(Of Scatter)(Function(s) True)
        chartDataCol.SetPlotData(chartDatafull)
    ...
    '            If drow("spotvalue") >= 1000 Then
    '                '            objProfitLossChart.chtprofit.RowLabel = Int(drow("spotvalue"))
    '            Else
    ...
    Private Sub create_chart_delta()
        ...
        Dim chartData As CChartData = chartDataCol.mChartDataDelta
        Dim chaDbl(deltatable.Rows.Count - 1, deltatable.Columns.Count - 3) As Double
        Dim xData(deltatable.Rows.Count) As Double
        ...
        If deltatable.Columns.Count > 2 Then
            ...
            Dim x As Integer = cha.GetLength(0)
            Dim y As Integer = cha.GetLength(1)



            'Dim clonedArray(cha(0, 0).Length - 1) As Double
            ...
            For Each col As DataColumn In deltatable.Columns
                If UCase(col.ColumnName) <> UCase("spotvalue") And UCase(col.ColumnName) <> UCase("Percent(%)") Then
                    ...
                    chartData.mLegendList.Add(col.ColumnName)
                    'Debug.WriteLine(chtdelta.ColumnLabel)
                    i += 1
                    c += 1
            ...
            Next
            r = 1
            Dim rowLable As String
            For Each drow As DataRow In deltatable.Rows
                'chtdelta.Row = r
                'chtdelta.RowLabel = drow("spotvalue")
                If drow("spotvalue") >= 1000 Then
                    rowLable = Int(drow("spotvalue"))
                    '             chtdelta.RowLabel = Int(drow("spotvalue"))
                Else
                    '            chtdelta.RowLabel = drow("spotvalue")
                    rowLable = drow("spotvalue")
                End If
                xData(r - 1) = CDbl(rowLable)
                r += 1
            Next
        End If
        ...
        Dim chartDatafull As CChartData = objProfitLossChart.chartDataCol.mChartDataDelta
        chartDatafull.mLegendList = chartData.mLegendList
        chartDatafull.mYDataLines = chaDbl
        chartDatafull.mXData = xData
        chartDatafull.mFormPlot.Plot.Remove(Of Scatter)(Function(s) True)
    ...
    '            If drow("spotvalue") >= 1000 Then
    '                objProfitLossChart.chtdelta.RowLabel = Int(drow("spotvalue"))
    '            Else
    ...
    Private Sub create_chart_gamma()
        ...
        Dim chartData As CChartData = chartDataCol.mChartDataGama
        Dim chaDbl(gammatable.Rows.Count - 1, gammatable.Columns.Count - 3) As Double
        Dim xData(gammatable.Rows.Count) As Double
        ...
        If gammatable.Columns.Count > 2 Then
            ...
            For Each drow As DataRow In gammatable.Rows
                If drow("spotvalue") >= 1000 Then
                    '          chtgamma.RowLabel = Int(drow("spotvalue"))
                    rowLbl = Int(drow("spotvalue"))
                Else
        ...
        Dim chartDatafull As CChartData = objProfitLossChart.chartDataCol.mChartDataGama
        chartDatafull.mLegendList = chartData.mLegendList
        chartDatafull.mYDataLines = chaDbl
        chartDatafull.mXData = xData
        chartDatafull.mFormPlot.Plot.Remove(Of Scatter)(Function(s) True)
    ...
    '            If drow("spotvalue") >= 1000 Then
    '                objProfitLossChart.chtgamma.RowLabel = Int(drow("spotvalue"))
    '            Else
    ...
    Private Sub create_chart_vega()
        ...
        Dim chartData As CChartData = chartDataCol.mChartDataVega
        Dim chaDbl(vegatable.Rows.Count - 1, vegatable.Columns.Count - 3) As Double
        Dim xData(vegatable.Rows.Count) As Double
        ...
        If vegatable.Columns.Count > 2 Then
            ...
            For Each drow As DataRow In vegatable.Rows
                ...
                If drow("spotvalue") >= 1000 Then
                    '        chtvega.RowLabel = Int(drow("spotvalue"))
                    xData(r - 1) = CDbl(Int(drow("spotvalue")))
                Else
                    '       chtvega.RowLabel = drow("spotvalue")
                    xData(r - 1) = CDbl(drow("spotvalue"))
                End If
                r += 1
            Next
        End If
        ...
        Dim chartDatafull As CChartData = objProfitLossChart.chartDataCol.mChartDataVega
        chartDatafull.mLegendList = chartData.mLegendList
        chartDatafull.mYDataLines = chaDbl
        chartDatafull.mXData = xData
        chartDatafull.mFormPlot.Plot.Remove(Of Scatter)(Function(s) True)
    ...
    '            If drow("spotvalue") >= 1000 Then
    '                objProfitLossChart.chtvega.RowLabel = Int(drow("spotvalue"))
    '            Else
    ...
    Private Sub create_chart_theta()
        ...
        Dim chartData As CChartData = chartDataCol.mChartDataTheta
        Dim chaDbl(thetatable.Rows.Count - 1, thetatable.Columns.Count - 3) As Double
        Dim xData(thetatable.Rows.Count) As Double
        ...
        If thetatable.Columns.Count > 2 Then
            ...
            For Each drow As DataRow In thetatable.Rows
                ...
                If drow("spotvalue") >= 1000 Then
                    '          chttheta.RowLabel = Int(drow("spotvalue"))
                    rowLbl = Int(drow("spotvalue"))
                Else
                    '          chttheta.RowLabel = drow("spotvalue")
                    rowLbl = drow("spotvalue")
        ...
        Dim chartDatafull As CChartData = objProfitLossChart.chartDataCol.mChartDataTheta
        chartDatafull.mLegendList = chartData.mLegendList
        chartDatafull.mYDataLines = chaDbl
        chartDatafull.mXData = xData
        chartDatafull.mFormPlot.Plot.Remove(Of Scatter)(Function(s) True)
    ...
    '            If drow("spotvalue") >= 1000 Then
    '                objProfitLossChart.chttheta.RowLabel = Int(drow("spotvalue"))
    '            Else
    ...
    Private Sub create_chart_volga()
        ...
        Dim chartData As CChartData = chartDataCol.mChartDataVolga
        Dim chaDbl(volgatable.Rows.Count - 1, volgatable.Columns.Count - 3) As Double
        Dim xData(volgatable.Rows.Count) As Double
        ...
        If volgatable.Columns.Count > 2 Then
            ...
            For Each drow As DataRow In volgatable.Rows
                ...
                If drow("spotvalue") >= 1000 Then
                    '           chtvolga.RowLabel = Int(drow("spotvalue"))
                    rowLbl = Int(drow("spotvalue"))
                Else
                    '           chtvolga.RowLabel = drow("spotvalue")
                    rowLbl = drow("spotvalue")
        ...
        Dim chartDatafull As CChartData = objProfitLossChart.chartDataCol.mChartDataVolga
        chartDatafull.mLegendList = chartData.mLegendList
        chartDatafull.mYDataLines = chaDbl
        chartDatafull.mXData = xData
        chartDatafull.mFormPlot.Plot.Remove(Of Scatter)(Function(s) True)
    ...
    '            If drow("spotvalue") >= 1000 Then
    '                objProfitLossChart.chtvolga.RowLabel = Int(drow("spotvalue"))
    '            Else
    ...
    Private Sub create_chart_vanna()
        ...
        Dim chartData As CChartData = chartDataCol.mChartDataVanna
        Dim chaDbl(vannatable.Rows.Count - 1, vannatable.Columns.Count - 3) As Double
        Dim xData(vannatable.Rows.Count) As Double
        ...
        If vannatable.Columns.Count > 2 Then
            ...
            For Each drow As DataRow In vannatable.Rows
                ...
                If drow("spotvalue") >= 1000 Then
                    '          chtvanna.RowLabel = Int(drow("spotvalue"))
                    rowLbl = Int(drow("spotvalue"))
                Else
        ...
        Dim chartDatafull As CChartData = objProfitLossChart.chartDataCol.mChartDataVanna
        chartDatafull.mLegendList = chartData.mLegendList
        chartDatafull.mYDataLines = chaDbl
        chartDatafull.mXData = xData
        chartDatafull.mFormPlot.Plot.Remove(Of Scatter)(Function(s) True)
    ...
    '            If drow("spotvalue") >= 1000 Then
    '                objProfitLossChart.chtvanna.RowLabel = Int(drow("spotvalue"))
    '            Else
        ...
        Select Case ind
            Case 0
                ...
                For Each grow As DataGridViewRow In grdtrad.Rows
                    ...
                    If CBool(grow.Cells("Active").Value) = True And Val(grow.Cells("units").Value) <> 0 Then
                        Dim SpotValue As Double = Val(profit.Rows(rind)(0)) + Val(grow.Cells("DifFactor").Value)
                        drow = rtable.NewRow
                        ...
                        Else
                            ...
                            If _mT = 0.0001 Then
                                ...
                                End If
                                drow("pl") = Math.Round(((Val(drow("Price")) - Val(grow.Cells("ltp").Value)) * Val(grow.Cells("units").Value)), 2)  ' 9 is used
                            ...
                            Else
                                bval = Greeks.Black_Scholes(SpotValue, Val(grow.Cells("Strike").Value), Mrateofinterast, 0, (Val(grow.Cells("lv").Value) / 100), _mT, iscall, True, 0, 1)
                                drow("delta") = Math.Round(Val(bval & ""), DecimalSetting.iRDelta)
                                drow("deltaval") = Math.Round(Val(bval & "") * Val(grow.Cells("units").Value), DecimalSetting.iRDeltaval)
                                bval = Greeks.Black_Scholes(SpotValue, Val(grow.Cells("Strike").Value), Mrateofinterast, 0, (Val(grow.Cells("lv").Value) / 100), _mT, iscall, True, 0, 4)
                                drow("theta") = Math.Round(Val(bval & ""), DecimalSetting.iRTheta)
                                drow("thetaval") = Math.Round(Val(bval & "") * Val(grow.Cells("units").Value), DecimalSetting.iRThetaval)
                                bval = Greeks.Black_Scholes(SpotValue, Val(grow.Cells("Strike").Value), Mrateofinterast, 0, (Val(grow.Cells("lv").Value) / 100), _mT, iscall, True, 0, 3)
                                drow("vega") = Math.Round(Val(bval & ""), DecimalSetting.iRVega)
                                drow("vgval") = Math.Round(Val(bval & "") * Val(grow.Cells("units").Value), DecimalSetting.iRVegaval)
                                bval = Greeks.Black_Scholes(SpotValue, Val(grow.Cells("Strike").Value), Mrateofinterast, 0, (Val(grow.Cells("lv").Value) / 100), _mT, iscall, True, 0, 2)
                                drow("gamma") = Math.Round(Val(bval & ""), DecimalSetting.iRGamma)
                                drow("gmval") = Math.Round(Val(bval & "") * Val(grow.Cells("units").Value), DecimalSetting.iRGammaval)

                                'Volga , Vanna
                                mVolatility = Greeks.Black_Scholes(SpotValue, Val(grow.Cells("Strike").Value), Mrateofinterast, 0, (Val(grow.Cells("lv").Value) / 100), _mT, iscall, True, 0, 6)
                                ...
                                bval = Greeks.Black_Scholes(SpotValue, Val(grow.Cells("Strike").Value), Mrateofinterast, 0, (Val(grow.Cells("lv").Value) / 100), _mT, iscall, True, 0, 7)
                                drow("vanna") = Math.Round(Val(bval & ""), DecimalSetting.iRVanna)
                                drow("vannaval") = Math.Round(Val(bval & "") * Val(grow.Cells("units").Value), DecimalSetting.iRVannaval)

                                bval = Greeks.Black_Scholes(SpotValue, Val(grow.Cells("Strike").Value), Mrateofinterast, 0, (Val(grow.Cells("lv").Value) / 100), _mT, iscall, True, 0, 8)
                                drow("volga") = Math.Round(Val(bval & ""), DecimalSetting.iRVolga)
                                drow("volgaval") = Math.Round(Val(bval & "") * Val(grow.Cells("units").Value), DecimalSetting.iRVolgaval)
                                ...
                                drow("pl") = Math.Round(CDbl(((Val(bval & "") - Val(grow.Cells("ltp").Value)) * Val(grow.Cells("units").Value))), 2)  ' 9 is used
                                drow("Price") = bval
...
#Region "Init"
    ...
    Private Sub init_table()
        ...
        If Val(txtllimit.Text > 0) And Val(interval) > 0 And Val(txtmid) > 0 Then
            ...
            Dim dt As DataTable = grdact.DataSource
            Dim tempcopy As New DataTable
            tempcopy.Columns.Add("Ddate", GetType(Date))
            ...
            For Each dr As DataRow In dataTable.Rows
                If (datadt.Columns.Contains(CDate(dr("Ddate")).ToString("dd/MMM/yyyy")) = False) Then

                    datadt.Columns.Add(CDate(dr("Ddate")).ToString("dd/MMM/yyyy"), GetType(Date))
                End If
            Next
            Try
                For i22 As Integer = 0 To datadt.Columns.Count - 1
                    If dt.Columns.Contains(datadt.Columns(i22).ColumnName) Then
                        'temptable.Columns(dataTable.Columns(i).ColumnName.ToString()).SetOrdinal = i
                        dt.Columns(datadt.Columns(i22).ColumnName.ToString).SetOrdinal(i22)
                    End If
                Next
            Catch ex As Exception
                ...
                For Each gcol As DataGridViewCheckBoxColumn In grdact.Columns
                    If Not IsDBNull(grow.Cells(gcol.Index).Value) Then
                        ...
                        If grow.Cells(gcol.Index).Value = True Then
                            '    If gcol.DataPropertyName.Split(" ")(0) <> "Expiry" Then
                            '        Dim tdate As Date = DateFormat(gcol.DataPropertyName)
            ...
            For j = 1 To Val(txtllimit.Text) + 1
                ...
                If chkint.Checked Then
                    drow("spotvalue") = txtmid - (Val((Val(txtinterval.Text) * txtmid / 100) * (Val(txtllimit.Text) - int)))
                Else
                ...
                If chkint.Checked = True Then
                    ...
                    drow("spotvalue") = Math.Round((txtmid * (100 + (-inter * 100))) / 100, 2)
            ...
            For j = 1 To Val(txtllimit.Text)
                ...
                If chkint.Checked = True Then
                    'inter = (Val(txtinterval.Text) * Val(j)) / 100 'comment by payal patel
                    inter = ((Val(txtinterval.Text) * (Val(j) + int)) / 100)
                    drow("Percent(%)") = inter * 100 '& " %"
        ...
        'objProfitLossChart.grdprofit.Columns.Clear()
        'For Each c As DataGridViewColumn In grdprofit.Columns
        '    objProfitLossChart.grdprofit.Columns.Add(c)
        'Next

        'objProfitLossChart.grddelta.Columns.Clear()
        'For Each c As DataGridViewColumn In grddelta.Columns
        '    objProfitLossChart.grddelta.Columns.Add(c)
        'Next

        'objProfitLossChart.grdgamma.Columns.Clear()
        'For Each c As DataGridViewColumn In grdgamma.Columns
        '    objProfitLossChart.grdgamma.Columns.Add(c)
        'Next

        'objProfitLossChart.grdvega.Columns.Clear()
        'For Each c As DataGridViewColumn In grdvega.Columns
        '    objProfitLossChart.grdvega.Columns.Add(c)
        'Next

        'objProfitLossChart.grdtheta.Columns.Clear()
        'For Each c As DataGridViewColumn In grdtheta.Columns
        '    objProfitLossChart.grdtheta.Columns.Add(c)
        'Next

        'objProfitLossChart.grdvolga.Columns.Clear()
        'For Each c As DataGridViewColumn In grdvolga.Columns
        '    objProfitLossChart.grdvolga.Columns.Add(c)
        'Next

        'objProfitLossChart.grdvanna.Columns.Clear()
        'For Each c As DataGridViewColumn In grdvanna.Columns
        '    objProfitLossChart.grdvanna.Columns.Add(c)
        'Next
...
#Region "Grid Event"
    Private Sub cal(ByVal cno As Integer, ByVal currow As DataGridViewRow, Optional ByVal check As Boolean = False)
        ...
        If Not IsDBNull(grow.Cells("CPF").Value) And Not grow.Cells("CPF").Value Is Nothing Then
            If UCase(grow.Cells("CPF").Value) = "F" Or UCase(grow.Cells("CPF").Value) = "E" Then
                ...
                If Val(grow.Cells("last").Value) = 0 Then
                    If Val(grow.Cells("spval").Value) <> 0 Then
                        grow.Cells("last").Value = Math.Round(Val(grow.Cells("spval").Value), 2)
                    Else
            ...
            Else
                ...
                If CDate(grow.Cells("TimeI").Value) < CDate(grow.Cells("TimeII").Value) Then
                    If INCLUDEEXPIRY_VOLANDGREEK_CAL = 1 Then
                        mt = (DateDiff(DateInterval.Day, CDate(grow.Cells("TimeI").Value).Date, CDate(grow.Cells("TimeII").Value).Date)) + 1
                    Else
        ...
        End If
    End Sub
    Private Function CCDate(ByVal ddate As Date) As String
        Try
            ...
            'Else
            Try
                If ValidateEDateForError(ddate) Then
                    ' Dim returnError As Boolean
                    '   Dim dateVal As Date
                    '  Dim formats() As String = {"dd/MM/yyyy", "dd/M/yyyy", "d/M/yyyy"}

                    Return CDate(ddate.ToString("dd-MMM-yy"))
                Else
                    Return ""
                End If
                'ddt = DateSerial(arr(2), arr(1), arr(0))
                'If IsDate(arr(1) & "/" & arr(0) & "/" & arr(2)) Then
                '    Return CDate(arr(1) & "/" & arr(0) & "/" & arr(2)).ToString("dd/MMM/yy")
                'Else
                '    Return ""
                'End If
            Catch ex As Exception
        ...
        End Try
    End Function
    Public Function ValidateEDateForError(ByVal checkInputValue As Date) As Boolean
        Dim returnError As Boolean
        Dim dateVal As Date
        ' Dim formats() As String = {"dd/MM/yyyy", "dd/M/yyyy", "d/M/yyyy"}
        Dim formats() As String = {"dd-MMM-yyyy", "dd-MMM-yyyy", "d-MMM-yyyy", "dd/MMM/yyyy", "d/MMM/yyyy", "dd/MM/yyyy", "dd/M/yyyy", "d/M/yyyy", "dd.MMM.yyyy", "d.MMM.yyyy"}
        If DateTime.TryParseExact(checkInputValue.ToString("dd/MMM/yyyy"), formats,
        ...
        Return returnError
    End Function
    Private Function IISdate(ByVal ddate As Date) As Boolean
        Try
            If ValidateEDateForError(ddate) Then
                Return True
            Else
    ...
    Private Sub grdtrad_CellEndEdit(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grdtrad.CellEndEdit
        ...
        Dim zero As Double = 0
        Dim one As Double = 1
        ' Dim dt As Date = CDate(grdtrad.Cells("TimeII").Value).Date
        ' Me.grdtrad.Columns("TimeII").DefaultCellStyle.Format = "MM/ddyyyy"
        If Not IISdate(grdtrad.Rows(e.RowIndex).Cells("TimeI").Value) Then grdtrad.Rows(e.RowIndex).Cells("TimeI").Value = dttoday.Value
        Try
            If Not IISdate(grdtrad.Rows(e.RowIndex).Cells("TimeII").Value) Then grdtrad.Rows(e.RowIndex).Cells("TimeII").Value = dtexp.Value
        Catch ex As Exception
            ...
            If Not IsDBNull(grdtrad.Rows(e.RowIndex).Cells(e.ColumnIndex).Value) Then
                If UCase(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) = "C" Or UCase(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) = "P" Or UCase(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) = "F" Or UCase(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) = "E" Then
                    If UCase(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) = "F" Or UCase(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) = "E" Then
                        ...
                        grdtrad.Rows(e.RowIndex).Cells("last").Value = Val(txtmkt.Text)
                    Else
        ...
        ElseIf grdtrad.Columns(e.ColumnIndex).Name = "spval" Then
            If IsDBNull(grdtrad.Rows(e.RowIndex).Cells("spval").Value) Or grdtrad.Rows(e.RowIndex).Cells("spval").Value Is Nothing Or CStr(grdtrad.Rows(e.RowIndex).Cells("spval").Value) = "" Then
                If UCase(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) = "F" Or UCase(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) = "E" Then
                    grdtrad.Rows(e.RowIndex).Cells("spval").Value = Val(txtmkt.Text)
                Else
        ...
        ElseIf grdtrad.Columns(e.ColumnIndex).Name = "Strike" Then
            ...
            Else
                grdtrad.Rows(e.RowIndex).Cells("Strike").Value = Math.Abs(Val(grdtrad.Rows(e.RowIndex).Cells("Strike").Value))
            End If
        ...
        ElseIf grdtrad.Columns(e.ColumnIndex).Name = "Active" Then
            ...
            If IsDBNull(grow.Cells("spval").Value) Or grow.Cells("spval").Value Is Nothing Then
                grdtrad.Rows(e.RowIndex).Cells("spval").Value = Val(txtmkt.Text)
            Else
                grow.Cells("spval").Value = Val(grow.Cells("spval").Value & "")
            End If
        ...
        Else
            If Val(grdtrad.CurrentRow.Cells("Strike").Value) > 0 And Val(grdtrad.CurrentRow.Cells("lv").Value) <> 0 And grdtrad.CurrentRow.Cells("Active").Value = True Then
                cal(0, grdtrad.CurrentRow)
            End If
        ...
        'result(False)
        Dim bval As Double
        If Val(grdtrad.CurrentRow.Cells("lv").Value & "") = 0 Then
            If grdtrad.CurrentRow.Cells("CPF").Value = "C" Then
                If ((Val(grdtrad.CurrentRow.Cells("spval").Value & "") - Val(grdtrad.CurrentRow.Cells("Strike").Value & "")) > 0) Then
                    bval = (Val(grdtrad.CurrentRow.Cells("spval").Value) - Val(grdtrad.CurrentRow.Cells("Strike").Value))
                    grdtrad.CurrentRow.Cells("last").Value = ((bval - Val(grdtrad.CurrentRow.Cells("ltp").Value & "")) * Val(grdtrad.CurrentRow.Cells("units").Value & ""))
                Else
                    bval = 0 ' CDbl(dr(col.ColumnName)) + ((0 - CDbl(grow.Cells(7).Value)) * CDbl(grow.Cells(6).Value))
                    grdtrad.CurrentRow.Cells("last").Value = ((bval - Val(grdtrad.CurrentRow.Cells("ltp").Value)) * Val(grdtrad.CurrentRow.Cells("units").Value))
                End If
            ElseIf grdtrad.CurrentRow.Cells("CPF").Value = "P" Then
                If ((Val(grdtrad.CurrentRow.Cells("Strike").Value) - Val(grdtrad.CurrentRow.Cells("spval").Value)) > 0) Then
                    bval = (Val(grdtrad.CurrentRow.Cells("Strike").Value) - Val(grdtrad.CurrentRow.Cells("spval").Value))
                    grdtrad.CurrentRow.Cells("last").Value = ((bval - Val(grdtrad.CurrentRow.Cells("ltp").Value)) * Val(grdtrad.CurrentRow.Cells("units").Value))
                Else
                    bval = 0 'CDbl(dr(col.ColumnName)) + ((0 - CDbl(grow.Cells(7).Value)) * CDbl(grow.Cells(6).Value))
                    grdtrad.CurrentRow.Cells("last").Value = ((bval - Val(grdtrad.CurrentRow.Cells("ltp").Value)) * Val(grdtrad.CurrentRow.Cells("units").Value))
    ...
    Private Sub grdtrad_RowsAdded(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewRowsAddedEventArgs) Handles grdtrad.RowsAdded
        ...
        grdtrad.Rows(e.RowIndex).Cells("TimeI").Value = dttoday.Value
        grdtrad.Rows(e.RowIndex).Cells("TimeII").Value = dtexp.Value
        'If grdtrad.Rows(e.RowIndex - 1).Cells("Active").Value = True Then
        'grdtrad.Rows(e.RowIndex).Cells("TimeII").Value = dtexp.Value
        'End If
    ...
    Private Sub grdtrad_CellFormatting(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellFormattingEventArgs) Handles grdtrad.CellFormatting
        If grdtrad.Rows(e.RowIndex).Cells("TimeII").Value = Nothing Or IsDBNull(grdtrad.Rows(e.RowIndex).Cells("TimeII").Value) Then
            grdtrad.Rows(e.RowIndex).Cells("TimeII").Value = dtexp.Value.Date
        End If
        'If IsDBNull(grdtrad.Rows(e.RowIndex).Cells("CPF").Value) Then grdtrad.Rows(e.RowIndex).Cells("CPF").Value = "C"
        If grdtrad.Rows(e.RowIndex).Cells("CPF").Value = "C" Or grdtrad.Rows(e.RowIndex).Cells("CPF").Value = "c" Then
            grdtrad.Rows(e.RowIndex).Cells("CPF").Value = "C"
            grdtrad.Rows(e.RowIndex).Cells("Active").Style.ForeColor = Color.Yellow
            grdtrad.Rows(e.RowIndex).Cells("TimeI").Style.ForeColor = Color.Yellow
...
#End Region
    ...
    Private Sub scenario1_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Write_ErrorLogScenario("Scenario Closed Start..")
        chkscenario = False
        txtmkt.Text = 0
        TxtMStrike.Text = 0
        'If Not objanalysis Is Nothing Then
        ...
        '    ObjSceDetail.Delete_scenario(cnstAnalysisScenario)
        '    ObjSceDetail.ScenarioName = cnstAnalysisScenario
        ...
        Write_ErrorLogScenario("Scenario Closed Stop..")

        'If ScenarioName <> "Analysis-Scenario" Then
        '    ObjSceDetail.ScenarioName = ScenarioName
    ...
    Private Sub scenario1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        ElseIf e.KeyCode = Keys.F7 Then
            If ShowPanel3 = False Then
                ShowPanel3 = True
                KeyF7Togal = True
                'AutoHideToolStripMenuItem.Checked = False
            Else
                KeyF7Togal = False
                BtnFixPnl3_Click(sender, e)
                ShowPanel3 = False
                'AutoHideToolStripMenuItem.Checked = True
            End If
        ...
        ElseIf e.KeyCode = Keys.Escape And TabLPanelSpotDiff.Visible = True Then
            ...
            'ElseIf e.KeyCode = Keys.Escape And TabLPanelDecSetting.Visible = True Then
            '    'TabLPanelDecSetting.Visible = False
    ...
    Private Sub scenario1_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        'If (flgmarketwatch = False) Then
        Write_ErrorLogScenario("Load Scenario..")

        '=========================REM:Coding For Get Interval by payal patel on 16/06/2014===============
        'If chkint.Checked = False Then

        '    Dim dtCurTlb As DataTable
        '    Dim CurrentStrike, NextStrike As Double
        '    Dim cp, company As String
        '    Dim mdate As Date
        '    Dim Dv As DataView

        '    Dv = New DataView(analysis.currtable, "Iscalc = True", "company", DataViewRowState.CurrentRows)
        '    dtCurTlb = Dv.ToTable()
        '    ' currentstrike = dtCurTlb.Rows(0).Item("strikes").ToString()
        '    For Each Dr As DataRow In dtCurTlb.Rows

        '        Dr = dtCurTlb.Select(" script='" & Dr("script") & "' and cp='" & Dr("cp") & "' ")(0)
        '        '    CurrentStrike = Dr("strikes")
        '        '   cp = Dr("cp")
        '        company = GetSymbol(Dr("Company"))
        '        '  company = Dr("Company")
        '        mdate = Dr("mdate")
        '    Next
        '    Dim ddr As DataRow
        ...
        '    ' NextStrike = objExp.getstrike(CurrentStrike, script)
        '    For Each Dr As DataRow In cpfmaster.Rows
        '        If cpfmaster.Select(" symbol='" & company & "' and  expdate='" & mdate.ToString("dd/MMM/yyyy") & "'  ").Length > 0 Then

        '            Dim Dv1 As DataView
        '            Dv1 = New DataView(cpfmaster, "symbol='" & company & "' And expdate='" & mdate.ToString("dd/MMM/yyyy") & "' and option_type='CE'", "strike_price desc", DataViewRowState.CurrentRows)
        ...
        '                temp = temp + 1
        '                If temp = 1 Then
        '                    ddr = dt1.Select(" symbol ='" & company & "'and  expdate='" & mdate.ToString("dd/MMM/yyyy") & "' ")(0)

        '                    CurrentStrike = Dr3("strike_price")


        '                ElseIf temp = 2 Then
        '                    NextStrike = Dr3("strike_price")
        '                    Exit For
        '                End If
        '            Next
        '            Exit For
        ...
        '    txtinterval.Text = CurrentStrike - NextStrike

        'Else
        '    txtinterval.Text = 2
        'End If
        ''  =======================REM:End========================================================
        'Me.grdtrad.Columns("TimeII").DefaultCellStyle.Format = "MM/dd/yyyy"

        Me.TabLPanelMain.RowStyles.Item(0) = (New System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Absolute, 30.0!))
        ...
        If scenariotable.Rows.Count > 0 Then
            ...
            '  grossmtm = 0
        End If
        TxtNoDate.Text = SCE_DAYS
        If VarIsCurrency = False Then
            txtinterast.Text = Val(IIf(IsDBNull(GdtSettings.Compute("max(SettingKey)", "SettingName='Rateofinterest'")), 0, GdtSettings.Compute("max(SettingKey)", "SettingName='Rateofinterest'")))
        ElseIf VarIsCurrency = True Then
            txtinterast.Text = Val(IIf(IsDBNull(GdtSettings.Compute("max(SettingKey)", "SettingName='CurrrencyRateofinterest'")), 0, GdtSettings.Compute("max(SettingKey)", "SettingName='Rateofinterest'")))
        End If
        ...
        If trscen = True Then
            ...
            gcheck = True
            dttoday.Value = CDate(time1)
            dtexp.Value = CDate(time2)


            'dtexp.Value = CDate(scenariotable.Compute("Max(time2)", "")) 'time2
            txtdays.Text = DateDiff(DateInterval.Day, CDate(time1.Date), CDate(time2.Date))
            If CDate(dttoday.Value.Date) < CDate(dtexp.Value.Date) Then
                REM This code commented because of Day difference plus 1 days display
                'txtdays.Text = Val(txtdays.Text) + 1
            ElseIf CDate(dttoday.Value.Date) = CDate(dtexp.Value.Date) Then
            ...
            Dim drscenario() As DataRow
            If scenariotable.Select("cpf<>'E'", "").Length > 0 Then
                grdtrad.Columns("TimeII").DefaultCellStyle.NullValue = Format(scenariotable.Select("cpf<>'E'", "")(0).Item("time2"), "MM/dd/yyyy")
            Else
                grdtrad.Columns("TimeII").DefaultCellStyle.NullValue = Format(time2, "MM/dd/yyyy")
            End If
            Dim MinExpDate As DateTime = scenariotable.Compute("Min(time2)", "")
            Dim mmValue As Double = scenariotable.Compute("Max(Spot)", "time2='" & MinExpDate.ToString("dd-MMM-yyyy") & "'")
            mvalue = mmValue
            txtmkt.Text = mvalue 'Math.Round(mvalue, 0)
            TxtMStrike.Text = mvalue 'Math.Round(mvalue, 0)
            txtmid = mvalue
            drscenario = scenariotable.Select("", "cpf")
            For Each drow As DataRow In drscenario
                ...
                If drow("cpf").ToString() = "E" Then
                    grdtrad.Rows(i).Cells("TimeII").Value = time2.Date
                    grdtrad.Rows(i).Cells("spval").Value = Val(txtmkt.Text)
                Else
                    grdtrad.Rows(i).Cells("TimeII").Value = CDate(drow("time2"))
                    grdtrad.Rows(i).Cells("spval").Value = Val(drow("spot"))
                End If

                grdtrad.Rows(i).Cells("CPF").Value = CStr(drow("cpf"))
                'grdtrad.Rows(i).Cells("spval").Value = Val(drow("spot"))
                grdtrad.Rows(i).Cells("Strike").Value = Val(drow("strikes"))
                grdtrad.Rows(i).Cells("units").Value = Val(drow("qty"))
                grdtrad.Rows(i).Cells("ltp").Value = Val(drow("ltp") & "")
                grdtrad.Rows(i).Cells("last").Value = Val(drow("rate") & "")
                If Val(drow("vol")) = 0 Then
                    grdtrad.Rows(i).Cells("lv").Value = Val(0.0)
                Else
                    grdtrad.Rows(i).Cells("lv").Value = Val(drow("vol"))
                End If
                grdtrad.Rows(i).Cells("delta").Value = Val(drow("delta"))
            ...
            Else
                grdtrad.Columns("TimeII").DefaultCellStyle.NullValue = Format(time2, "MM/dd/yyyy")
            End If
            MinExpDate = scenariotable.Compute("Min(time2)", "")
            mmValue = scenariotable.Compute("Max(Spot)", "time2='" & MinExpDate.ToString("dd-MMM-yyyy") & "'")
            mvalue = mmValue
            txtmkt.Text = mvalue 'Math.Round(mvalue, 0)
            TxtMStrike.Text = mvalue 'Math.Round(mvalue, 0)
            txtmid = mvalue
        ...
        ''objScenarioDetail.ScenarioName = ""
        ...
        txtdays.Text = 0
        If CDate(dttoday.Value.Date) <= CDate(dtexp.Value.Date) Then
            txtdays.Text = (DateDiff(DateInterval.Day, CDate(dttoday.Value.Date), CDate(dtexp.Value.Date)))
        End If
        If CDate(dttoday.Value.Date) < CDate(dtexp.Value.Date) Then
            REM This code commented because of Day difference plus 1 days display
            'txtdays.Text = Val(txtdays.Text) + 1
        ...
        frmFlg = False
        create_active()
        LoadSelectedDate(ObjSceDetail.SelectedDate)
        ...
        '    If ScenarioName = cnstAnalysisScenario Or markerstring <> "" Then
        '        SaveTag(ScenarioName)
        '    End If
        'Else
        ...
        Write_ErrorLogScenario("Scenario Load End..")
    ...
    Private Sub ChartInit(pChartColl As CChartCollection)
        ...
        pChartColl.mChartDataProfitLoss.mFormPlot.Tag = "profit"
        AddHandler pChartColl.mChartDataProfitLoss.mFormPlot.DoubleClick, AddressOf chartPNL_DoubleClick
        ...
        pChartColl.mChartDataDelta.mFormPlot.Tag = "delta"
        AddHandler pChartColl.mChartDataDelta.mFormPlot.DoubleClick, AddressOf chartPNL_DoubleClick
        '------------------------------------------------------------------------
        ...
        pChartColl.mChartDataGama.mFormPlot.Tag = "gamma"
        AddHandler pChartColl.mChartDataGama.mFormPlot.DoubleClick, AddressOf chartPNL_DoubleClick
        '------------------------------------------------------------------------
        ...
        pChartColl.mChartDataTheta.mFormPlot.Tag = "theta"
        AddHandler pChartColl.mChartDataTheta.mFormPlot.DoubleClick, AddressOf chartPNL_DoubleClick
        '------------------------------------------------------------------------
        ...
        pChartColl.mChartDataVega.mFormPlot.Tag = "vega"
        AddHandler pChartColl.mChartDataVega.mFormPlot.DoubleClick, AddressOf chartPNL_DoubleClick
        ...
        pChartColl.mChartDataVolga.mFormPlot.Tag = "volga"
        AddHandler pChartColl.mChartDataVolga.mFormPlot.DoubleClick, AddressOf chartPNL_DoubleClick
        ...
        pChartColl.mChartDataVanna.mFormPlot.Tag = "vanna"
        AddHandler pChartColl.mChartDataVanna.mFormPlot.DoubleClick, AddressOf chartPNL_DoubleClick
    ...
    Private Sub SplitContainer1_Panel1_MouseHover(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SplitContainer1.Panel1.MouseHover, grdtrad.MouseHover
        ...
        '    End If
        'End If
        'If KeyF7Togal = False Then
        '    If MousePosition.X <= 55 Then
        '        ShowPanel3 = True
        '    Else
        '        ShowPanel3 = False
        '    End If
    ...
    Private Sub Timer1_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer1.Tick
        ...
        If TmPnl2 = True Then
            If ShowPanel2 = True Then
                ...
                If TabLPanel2.Location.Y >= ((SplitContainer1.Panel2.Height - 331) + 305) Then '270
                    If TabLPanel2.Location.Y = ((SplitContainer1.Panel2.Height - 331) + 305) Then '270
                        grdStaticCalc.Focus()
            ...
            Else
                If TabLPanel2.Location.Y <= ((SplitContainer1.Panel2.Height - 331) + 350) Then '350
        ...
        If TmPnl3 = True Then
            If ShowPanel3 = True Then
                If TabLPanel3.Location.X <= -10 Then
                    If TabLPanel3.Location.X = 0 Then
                        'txtmkt.Focus()
                    ...
                    TabLPanel3.Left = TabLPanel3.Location.X + 10
                End If
            Else
                If TabLPanel3.Location.X >= -180 Then
                    If TabLPanel3.Location.X = -180 Then
                        'grdtrad.Focus()
                    End If
                    TabLPanel3.Left = TabLPanel3.Location.X - 10
    ...
    Private Sub BtnFix_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnFix.Click
        ...
        Else
            TmPnl1 = True
            'Timer1.Start()
            Me.TabLPanelMain.RowStyles.Item(0) = (New System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Percent, 1.0!))
    ...
    Private Sub BtnFixPnl3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        'If TmPnl3 = True Then
        '    'Timer1.Stop()
        '    TmPnl3 = False
        '    'Me.TableLayoutPanel1.RowStyles.Item(0) = (New System.Windows.Forms.RowStyle(System.Windows.Forms.SizeType.Absolute, 30.0!))
        ...
        If Val(txtmkt.Text) > 0 Then
            ...
            If grdtrad.Rows.Count > 0 Then
                For i As Integer = 0 To grdtrad.Rows.Count - 2
                    Dim grow As DataGridViewRow
                    grow = grdtrad.Rows(i)
                    grow.Cells("spval").Value = Val(txtmkt.Text) + grow.Cells("DifFactor").Value
                    ...
                    cal(7, grow, True)
                Next
                grdtrad.Rows(grdtrad.Rows.Count - 1).Cells("spval").Value = Val(txtmkt.Text) + grdtrad.Rows(grdtrad.Rows.Count - 1).Cells("DifFactor").Value
                grdtrad.EndEdit()
                ' result()
            End If
        End If
    ...
    Private Sub cmdcheck_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdcheck.Click
        Write_ErrorLogScenario("Check Start..")
        bCheckAll = True
        ...
        Write_ErrorLogScenario("Check Stop..")
    End Sub
    ...
    Private Sub txtdays_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If txtdays.Text.Trim <> "" And txtdays.Text <> "0" Then
            dtexp.Value = DateAdd(DateInterval.Day, CInt(txtdays.Text), CDate(dttoday.Value))
        End If
        cal_summary()
    ...
    Private Sub txtcvol_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) 'Handles txtcvol.Leave
        ...
        If Val(txtcvol.Text) > 0 Then
            If grdtrad.Rows.Count > 0 Then
                ...
                grdtrad.Rows(grdtrad.Rows.Count - 1).Cells("lv").Value = Val(txtcvol.Text)
                grdtrad.EndEdit()
                'End If
                cal_summary()
            End If
    ...
    Private Sub txtinterval_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtinterval.Leave 'Handles txtinterval.Leave
        ...
        '        txtinterval.Text = 2
        '    End If
        '    If txtinterval.Text > 100 Then
        '        MessageBox.Show("Enter Value Between 1 to 100")
        '        txtinterval.Text = ""
        '        txtinterval.Focus()
        '    End If
        'End If
        ...
        If Flag = True Then
            ...
            If Double.TryParse(txtinterval.Text, intervalValue) AndAlso intervalValue < 0.5 Then
                Dim result As DialogResult = MessageBox.Show("We recommend setting the interval above 0.5 for better performance of Volhedge. Do you still want to continue with your value?", "Performance Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning)

                If result = DialogResult.Cancel Then
                    txtinterval.Text = "0.5" ' Set interval to 0.5 if the user clicks Cancel
                End If
            End If
            Flag = False
        End If
    ...
    Private Sub chtprofit_DblClick(ByVal sender As Object, ByVal e As System.EventArgs)  ', grdprofit.DoubleClick
        chartName = "profit"
        ValLBLprofit = lblprofit.Text
        objProfitLossChart.ShowForm()
        ' TODO : 1. SHOW FORM CHART BIG
    End Sub
    Private Sub chttheta_DblClick(ByVal sender As Object, ByVal e As System.EventArgs)  ', grdtheta.DoubleClick
        chartName = "theta"
        ValLBLtheta = lbltheta.Text
        objProfitLossChart.ShowForm()
    End Sub

    Private Sub chtvega_DblClick(ByVal sender As Object, ByVal e As System.EventArgs)  ', grdvega.DoubleClick
        chartName = "vega"
        ValLBLvega = lblvega.Text
        objProfitLossChart.ShowForm()
    End Sub

    Private Sub chtdelta_DblClick(ByVal sender As Object, ByVal e As System.EventArgs)  ', grddelta.DoubleClick
        chartName = "delta"
        ValLBLdelta = lbldelta.Text
        objProfitLossChart.ShowForm()
    End Sub

    Private Sub chtgamma_DblClick(ByVal sender As Object, ByVal e As System.EventArgs)  ', grdgamma.DoubleClick
        chartName = "gamma"
        ValLBLgamma = lblgamma.Text
        objProfitLossChart.ShowForm()
    End Sub

    Private Sub chtvolga_DblClick(ByVal sender As Object, ByVal e As System.EventArgs)  ', grdvolga.DoubleClick
        chartName = "volga"
        ValLBLvolga = lblvolga.Text
        objProfitLossChart.ShowForm()
    End Sub
    Private Sub chtvanna_DblClick(ByVal sender As Object, ByVal e As System.EventArgs)  ', grdvanna.DoubleClick
        chartName = "vanna"
    ...
    Private Sub chkint_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkint.CheckedChanged
        ...
        '        Dv = New DataView(analysis.currtable, "Iscalc = True", "company", DataViewRowState.CurrentRows)
        '        dtCurTlb = Dv.ToTable()
        '        CurrentStrike = scenariotable.Rows(0).Item("strikes").ToString()
        '        For Each Dr As DataRow In dtCurTlb.Rows
        ...
        '        ' NextStrike = objExp.getstrike(CurrentStrike, script)
        '        For Each Dr As DataRow In cpfmaster.Rows
        '            If cpfmaster.Select(" symbol='" & company & "' and strike_price>'" & CurrentStrike & "' and expdate='" & mdate.ToString("dd/MMM/yyyy") & "'  ").Length > 0 Then
        '                ddr = cpfmaster.Select(" symbol ='" & company & "'and strike_price>'" & CurrentStrike & "' and expdate='" & mdate.ToString("dd/MMM/yyyy") & "' ")(0)

        '                NextStrike = ddr("strike_price")
        '            End If
        '            Exit For
        '        Next
        '        txtinterval.Text = NextStrike - CurrentStrike
        ...
        '        'Dv = New DataView(MarketWatch, "Iscalc = True", "company", DataViewRowState.CurrentRows)
        '        'dtCurTlb = Dv.ToTable()

        '        'For Each Dr As DataRow In MarketWatch.marketsce.Rows
        '        '    If Dr("cp") = "XX" Or Dr("cp") = "F" Or Dr("cp") = "X" Or Dr("cp") = "" Then
        '        '    Else
        '        '        Dr = dtCurTlb.Select(" script='" & Dr("script") & "' and cp='" & Dr("cp") & "' ")(0)
        '        '        CurrentStrike = Dr("strikes")
    ...
    End Sub
    Private Sub IncDecVolCal(ByVal iscall As Boolean) ''Viral
        Dim cp As String = ""
        Dim VolPM As Double = 0.0
        If txtvol.Text.Trim = "" Then
            txtvol.Text = 0
        End If
        If iscall = True Then
            cp = "C"
            VolPM = Val(txtcvolPM.Text)
        Else
            cp = "P"
            VolPM = Val(txtpvolPM.Text)
        End If
        If grdtrad.Rows.Count >= 0 Then
            For Each grow As DataGridViewRow In grdtrad.Rows

                If grow.Cells("CPF").Value = cp Then
                    If grow.Cells("lv").ReadOnly = False Then
                        If involflag = True Then
                            grow.Cells("lv").Value = Format(grow.Cells("lv").Value + VolPM, "#0.00")
                        ElseIf decvolflag = True Then
                            If grow.Cells("lv").Value - VolPM > 0 Then
                                grow.Cells("lv").Value = Format(grow.Cells("lv").Value - VolPM, "#0.00")
                            End If
                        End If
                    End If
    ...
    Private Sub tbcon_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbcon.SelectedIndexChanged
        ...
        Select Case UCase(tbcon.SelectedTab.Name)
            Case "TBPROFITCHART"
                ...
                If grdStaticCalc.ColumnCount <= 0 Then Exit Sub
                grdStaticCalc.Columns(grdStaticCalc.ColumnCount - 1).Visible = True
            Case "TBGRDPROFIT"
                If grdStaticCalc.ColumnCount <= 0 Then Exit Sub
                grdStaticCalc.Columns(grdStaticCalc.ColumnCount - 1).Visible = True
            Case "TBCHDELTA"
                ...
                If grdStaticCalc.ColumnCount <= 0 Then Exit Sub
                grdStaticCalc.Columns(grdStaticCalc.ColumnCount - 1).Visible = False
            Case "TBGRDDELTA"
                If grdStaticCalc.ColumnCount <= 0 Then Exit Sub
                grdStaticCalc.Columns(grdStaticCalc.ColumnCount - 1).Visible = False
            Case "TBCHGAMMA"
                ...
                For Each col In grdStaticCalc.Columns
                    If UCase(col.Name) <> UCase("spotvalue") And UCase(col.Name) <> UCase("Percent") Then
                        ...
                        For Each grow As DataGridViewRow In grdtrad.Rows
                            ...
                            If CBool(grow.Cells("Active").Value) = True And Val(grow.Cells("units").Value) <> 0 Then
                                SpotValue = Val(grdStaticCalc.Rows(RowIndex).Cells("SpotValue").Value & "") + Val(grow.Cells("DifFactor").Value & "")
                                If grow.Cells("CPF").Value = "F" Or grow.Cells("CPF").Value = "E" Then
                                    If Val(grow.Cells("spval").Value) = 0 Then
                                        'MsgBox("Enter Spot Value for Selected Future.")
                                        'grdtrad.Focus()
                                    ...
                                    End If
                                    'grdStaticCalc.Rows(RowIndex).Cells(col.Name).Value = (CDbl(grdStaticCalc.Rows(RowIndex).Cells(col.Name).Value) + ((CDbl(grdStaticCalc.Rows(RowIndex).Cells("SpotValue").Value) - CDbl(grow.Cells("ltp").Value)) * CDbl(grow.Cells("units").Value)))
                                    grdStaticCalc.Rows(RowIndex).Cells(col.Name).Value = (Val(grdStaticCalc.Rows(RowIndex).Cells(col.Name).Value) + ((SpotValue - Val(grow.Cells("ltp").Value)) * Val(grow.Cells("units").Value)))
    ...
    End Function
    Private Function GetWeekNumberOfMonth(ByVal dDt As Date) As Integer
        Dim dt As Date = DateSerial(dDt.AddMonths(1).Year, dDt.AddMonths(1).Month, 1).AddDays(-1)
        ...
        If weekNumber = 4 Then
            ...
            Dim deDiff As Integer = DateDiff(DateInterval.Day, LastDate, dt)
            If deDiff < 7 Then weekNumber = 5
        End If

        Return weekNumber
    End Function
    Public Function GetNthDayOfNthWeek(ByVal dt As Date, ByVal DayofWeek As Integer) As Date
        'specify which day of which week of a month and this function will get the date
        'this function uses the month and year of the date provided

        'getNoof week
        Dim WhichWeek As Integer = GetWeekNumberOfMonth(dt)
    ...
    Private Sub GetSpotDiff(ByVal bSet As Boolean)
        ...
        If NextMonth = 12 Then
            FarMonth = 1
        Else
            FarMonth = NextMonth + 1
        End If


        If (GetNthDayOfNthWeek(Now.Date, 4) < CDate(Date.Now)) Then


            ' If (DateAdd(DateInterval.Day, -(4 + (CInt(DateAdd(DateInterval.Day, -1, CDate(Date.Now.AddMonths(1).ToString("MMM/yyyy"))).DayOfWeek))), CDate(Date.Now.AddMonths(1).ToString("MMM/yyyy"))) < CDate(Date.Now)) Then
            If (DateAdd(DateInterval.Day, -(4 + (CInt(DateAdd(DateInterval.Day, -1, CDate(Date.Now.AddMonths(1).ToString("MMM/yyyy"))).DayOfWeek))), CDate(Date.Now.AddMonths(1).ToString("MMM/yyyy"))) <> fDate(Date.Now)) Then
                CurrMonth = Month(Date.Now) + 1
                'NextMonth = CurrMonth + 1
                'FarMonth = NextMonth + 1
                If CurrMonth >= 12 Then
                    CurrMonth = 12
        ...
        Catch ex As Exception
            ...
            Exit Sub
        End Try
        If Scenario_synFut = True Then

            'Dim DtFMonthDate As DataTable = New DataView(cpfmaster, "Symbol='NIFTY' AND option_type='XX'", "expdate1", DataViewRowState.CurrentRows).ToTable(True, "expdate1", "token")
            'Dim ftoken As Long
            'Dim currm, nextm, farm As Date
            'If DtFMonthDate.Rows.Count > 0 Then
            '    If UCase(GVarMaturity_Far_month) = "CURRENT MONTH" Then

            '        currm = DtFMonthDate.Rows(2)("expdate1")
            '    ElseIf UCase(GVarMaturity_Far_month) = "NEXT MONTH" Then

            '        nextm = DtFMonthDate.Rows(2)("expdate1")
            '    ElseIf UCase(GVarMaturity_Far_month) = "FAR MONTH" Then

            '        farm = DtFMonthDate.Rows(2)("expdate1")
            '    End If
            'End If
            ...
            For Each grow As DataGridViewRow In grdtrad.Rows
                If grow.Cells("Active").Value = True Then
                    ...
                    If CDate(grow.Cells("TimeII").Value).Month = CurrMonth Then
                        CurrFutPrice = Scenario_synFutCurr 'grow.Cells("spval").Value
                    ElseIf CDate(grow.Cells("TimeII").Value).Month = NextMonth Then
                        NextFutPrice = Scenario_synFutNext ' grow.Cells("spval").Value
                    ElseIf CDate(grow.Cells("TimeII").Value).Month = FarMonth Then
                        FarFutPrice = Scenario_synFutFar 'grow.Cells("spval").Value
                    End If
                End If
            Next
        ...
        If bSet = False Then
            ...
            If CurrFutPrice <> 0 Then
                'If (FarFutPrice - CurrFutPrice) > 0 Then
                TxtNextToFar.Text = Math.Round((FarFutPrice - CurrFutPrice), 4)
                'End If
            Else
                TxtNextToFar.Text = "0"
            End If
            ...
            For Each grow As DataGridViewRow In grdtrad.Rows
                If Scenario_synFut = True Then
                    ...
                    If grow.Cells("Active").Value = True Then
                        If CDate(grow.Cells("TimeII").Value) = currm Then
                            grow.Cells("DifFactor").Value = 0
                        ElseIf CDate(grow.Cells("TimeII").Value) = nextm Then
                            grow.Cells("DifFactor").Value = Val(TxtCurrToNext.Text)
                        ElseIf CDate(grow.Cells("TimeII").Value) = farm Then
                    ...
                    If grow.Cells("Active").Value = True Then
                        ...
                        If CDate(grow.Cells("TimeII").Value) < currm Then
                            grow.Cells("DifFactor").Value = Math.Round(grow.Cells("spval").Value - txtmkt.Text, 2)
                        ElseIf CDate(grow.Cells("TimeII").Value) < nextm Then
                            grow.Cells("DifFactor").Value = Math.Round(grow.Cells("spval").Value - txtmkt.Text, 2)
                        ElseIf CDate(grow.Cells("TimeII").Value) < farm Then
                            grow.Cells("DifFactor").Value = Math.Round(grow.Cells("spval").Value - txtmkt.Text, 2)
            ...
            If Val(txtmkt.Text) > 0 Then
                ...
                If grdtrad.Rows.Count > 0 Then
                    For i As Integer = 0 To grdtrad.Rows.Count - 2
                        ...
                        If grow.Cells("CPF").Value = "F" Or grow.Cells("CPF").Value = "E" Then
                            'If IsDBNull(grow.Cells(4).Value) Or grow.Cells(4).Value Is Nothing Or CStr(grow.Cells(4).Value) = "" Then
                            grow.Cells("last").Value = grow.Cells("spval").Value 'Val(txtmkt.Text)
                            'End If
                        End If
                        cal(7, grow, True)
                    Next
    ...
    Private Sub GetSpotDiff2(ByVal bSet As Boolean)
        Dim CurrFutPrice As Double
        Dim NextFutPrice As Double
        Dim FarFutPrice As Double
        Dim CurFarFutPrice As Double
        Dim NextFarFutPrice As Double
        Dim FarFarFutPrice As Double


        Dim CurrMonth As Integer = Month(Date.Now)
        Dim NextMonth As Integer = CurrMonth + 1
        Dim FarMonth As Integer = NextMonth + 1
        Dim CurFarMonth As Integer = FarMonth + 1
        Dim NextFarMonth As Integer = CurFarMonth + 1
        Dim FarFarMonth As Integer = NextFarMonth + 1
        ...
        For Each grow As DataGridViewRow In grdtrad.Rows
            If grow.Cells("Active").Value = True Then
                ...
                ElseIf CDate(grow.Cells("TimeII").Value).Month = FarMonth Then
                    FarFutPrice = grow.Cells("spval").Value
                    'ElseIf CDate(grow.Cells("TimeII").Value).Month = CurFarMonth Then
                    '    CurFarFutPrice = grow.Cells("spval").Value
                    'ElseIf CDate(grow.Cells("TimeII").Value).Month = NextFarMonth Then
                    '    NextFarFutPrice = grow.Cells("spval").Value
                    'ElseIf CDate(grow.Cells("TimeII").Value).Month = FarFarMonth Then
                    '    FarFarFutPrice = grow.Cells("spval").Value
                End If
            End If
        Next
        ...
        If bSet = False Then
            ...
            Dim FartoFar As Double = 0
            If FarFutPrice <> 0 Then
                If (CurFarFutPrice - FarFutPrice) > 0 Then
                    FartoFar = Math.Round((CurFarFutPrice - NextFutPrice), 4) 'FarFutPrice
                End If
            Else
                FartoFar = 0
            End If

            Dim CurFartoFar As Double = 0
            If CurFarFutPrice <> 0 Then
                If (NextFarFutPrice - CurFarFutPrice) > 0 Then
                    CurFartoFar = Math.Round((NextFarFutPrice - NextFutPrice), 4) 'CurFarFutPrice
                End If
            Else
                CurFartoFar = 0
            End If

            Dim NextFartoFar As Double = 0
            If NextFarFutPrice <> 0 Then
                If (FarFarFutPrice - NextFarFutPrice) > 0 Then
                    NextFartoFar = Math.Round((FarFarFutPrice - NextFutPrice), 4) 'NextFarFutPrice
                End If
            Else
                NextFartoFar = 0
            End If
            ...
            For Each grow As DataGridViewRow In grdtrad.Rows
                If grow.Cells("Active").Value = True Then
                    ...
                    ElseIf CDate(grow.Cells("TimeII").Value).Month = FarMonth Then
                        grow.Cells("DifFactor").Value = Val(TxtNextToFar.Text)
                        'ElseIf CDate(grow.Cells("TimeII").Value).Month = CurFarMonth Then
                        '    grow.Cells("DifFactor").Value = FartoFar
                        'ElseIf CDate(grow.Cells("TimeII").Value).Month = NextFarMonth Then
                        '    grow.Cells("DifFactor").Value = CurFartoFar
                        'ElseIf CDate(grow.Cells("TimeII").Value).Month = FarFarMonth Then
                        '    grow.Cells("DifFactor").Value = NextFartoFar
                    End If
                End If
            Next
        Else
            ...
            For Each grow As DataGridViewRow In grdtrad.Rows
                If grow.Cells("Active").Value = True Then
                    ...
                    ElseIf CDate(grow.Cells("TimeII").Value).Month = FarMonth Then
                        grow.Cells("DifFactor").Value = Val(TxtNextToFar.Text)
                        'ElseIf CDate(grow.Cells("TimeII").Value).Month = FarMonth Then
                        '    grow.Cells("DifFactor").Value = Val(TxtNextToFar.Text) 'FartoFar
                        'ElseIf CDate(grow.Cells("TimeII").Value).Month = FarMonth Then
                        '    grow.Cells("DifFactor").Value = Val(TxtNextToFar.Text) 'CurFartoFar
                        'ElseIf CDate(grow.Cells("TimeII").Value).Month = FarMonth Then
                        '    grow.Cells("DifFactor").Value = Val(TxtNextToFar.Text) 'NextFartoFar
                    End If
                End If
            Next
    ...
    'End Sub
    'Private Sub chtprofit_MouseDownEvent(ByVal sender As System.Object, ByVal e As AxMSChart20Lib._DMSChartEvents_MouseDownEvent)
    '    If e.button = 2 Then
    ...
    Private Sub dtexp_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles dtexp.Validating
        If CDate(dttoday.Value.Date) <= CDate(dtexp.Value.Date) Then
    ...
    Private Sub dtexp_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dtexp.ValueChanged
        'change by payalpatel
        'If dtexp.Value.Date < dttoday.Value.Date Then
        '    dtexp.Value = dttoday.Value.Date
        'End If
        'If grdtrad.Rows.Count > 1 Then
        grdtrad.Rows(grdtrad.Rows.Count - 1).Cells("TimeII").Value = dtexp.Value
        'End If
    ...
    Private Sub ResetToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ResetToolStripMenuItem.Click
        If MsgBox("Do You Want To Reset Scenario Position?", MsgBoxStyle.YesNo) = MsgBoxResult.No Then Exit Sub
        Try
    ...
    Private Sub dttoday_Leave1(ByVal sender As Object, ByVal e As System.EventArgs) Handles dttoday.Leave

    End Sub

    Private Sub dttoday_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles dttoday.Validating
        If CDate(dttoday.Value.Date) <= CDate(dtexp.Value.Date) Then
    ...
    Private Sub dttoday_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dttoday.ValueChanged
        ...
        'If CDate(dttoday.Value.Date) > CDate(dtexp.Value.Date) Then
        '    dttoday.Value = CDate(dtexp.Value.Date)
        'End If

        grdtrad.Rows(grdtrad.Rows.Count - 1).Cells("TimeI").Value = CDate(dttoday.Value)
        isStartEndDateChange = True
        txtdays.Text = 0
        If grdtrad.Rows.Count = 2 Then
            grdtrad.Rows(0).Cells("TimeI").Value = CDate(dttoday.Value)
        ElseIf frmFlg = True Then 'this flg used bcoz first need not to assign expiry date
            For Each grow As DataGridViewRow In grdtrad.Rows
                If grow.Cells("TimeI").ReadOnly = False Then
                    grow.Cells("TimeI").Value = dttoday.Value
    ...
    Private Sub TabLPanel1_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TabLPanel1.Leave
        ...
        If Flgtxtcvol = True Then
            Flgtxtcvol = False
            Call txtcvol_Leave(sender, e)
        End If
        If Flgtxtpvol = True Then
            Flgtxtpvol = False
    ...
    Private Sub txtmktPM_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtmktPM.Enter, txtpvolPM.Enter, txtdaysPM.Enter, txtcvolPM.Enter, TxtNoDate.Enter
        CType(sender, TextBox).BackColor = Color.FromArgb(255, 255, 0)

    End Sub

    Private Sub txtmktPM_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtmktPM.Leave, txtpvolPM.Leave, txtcvolPM.Leave, txtdaysPM.Leave
        CType(sender, TextBox).BackColor = Color.FromArgb(255, 255, 192)
    End Sub
    ...
    Private Sub txtmktPM_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtmktPM.TextChanged

    End Sub

    Private Sub BtnIncCmp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnIncCmp.Click, BtnIncCVol.Click, BtnIncPVol.Click, BtnIncDay.Click

        Select Case CType(sender, Button).Name
            Case "BtnIncCmp"
                txtmkt.Text = Val(txtmkt.Text & "") + Val(txtmktPM.Text)
                flgUnderline = True
                Call txtmkt_Leave(sender, e)
            Case "BtnIncCVol"
                'txtcvol.Text = Val(txtcvol.Text & "") + Val(txtcvolPM.Text)
                'Call txtcvol_Leave(sender, e)
                involflag = True
                Call IncDecVolCal(True)
            Case "BtnIncPVol"
                'txtpvol.Text = Val(txtpvol.Text & "") + Val(txtpvolPM.Text)
                'Call txtpvol_Leave(sender, e)
                involflag = True
                Call IncDecVolCal(False)
            Case "BtnIncDay"
                txtdays.Text = Val(txtdays.Text & "") - Val(txtdaysPM.Text)
                dttoday.Value = DateAdd(DateInterval.Day, -Val(txtdays.Text), CDate(dtexp.Value))
                frmFlg = False
                create_active()
        End Select
    ...
    Private Sub BtnDecCmp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDecCmp.Click, BtnDecCVol.Click, BtnDecPVol.Click, BtnDecDay.Click
        Select Case CType(sender, Button).Name
            Case "BtnDecCmp"
                txtmkt.Text = Val(txtmkt.Text & "") - Val(txtmktPM.Text)
                flgUnderline = True
    ...
    Private Sub txtmktPM_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtmktPM.KeyDown, txtcvolPM.KeyDown, txtpvolPM.KeyDown, txtdaysPM.KeyDown, TxtNoDate.KeyDown
        ...
        End If
    End Sub
    Private Sub txtmktPM_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtmktPM.KeyPress
        numonly(e)
    End Sub

    Private Sub txtcvolPM_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtcvolPM.KeyPress
        numonly(e)
    End Sub

    Private Sub txtpvolPM_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtpvolPM.KeyPress
        numonly(e)
    End Sub

    Private Sub txtdaysPM_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtdaysPM.KeyPress, TxtNoDate.KeyPress
        numonly(e)
    End Sub


    'Private Sub TabLPanelDecSetting_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabLPanelDecSetting.MouseClick
    '    DSleft = TabLPanelDecSetting.Left
    '    DSTop = TabLPanelDecSetting.Top
    'End Sub

    'Private Sub TabLPanelDecSetting_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabLPanelDecSetting.MouseDown
    '    'MsgBox(e.Clicks)
    '    FrmDSMove = True
    '    'TabLPanelSpotDiff.Left = tleft
    '    DragDS_X = e.X
    ...
    'Private Sub TabLPanelDecSetting_MouseMove(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabLPanelDecSetting.MouseMove
    '    'If MouseDnFlg = True Then
    '    '    TabLPanelSpotDiff.Left = e.X
    '    '    TabLPanelSpotDiff.Top = e.Y
    '    'End If
    ...
    '    Dim nx, ny
    '    If FrmDSMove Then
    '        nx = TabLPanelDecSetting.Left + e.X - DragDS_X
    '        ny = TabLPanelDecSetting.Top + e.Y - DragDS_Y
    '        TabLPanelDecSetting.Left = nx
    '        TabLPanelDecSetting.Top = ny
    '    End If

    'End Sub

    'Private Sub TabLPanelDecSetting_MouseUp(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles TabLPanelDecSetting.MouseUp
    '    Dim nx, ny
    '    nx = TabLPanelDecSetting.Left + e.X - DragDS_X
    '    ny = TabLPanelDecSetting.Top + e.Y - DragDS_Y
    '    TabLPanelDecSetting.Left = nx
    ...
    'Private Sub TabLPanelDecSetting_Paint(ByVal sender As System.Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles TabLPanelDecSetting.Paint

    'End Sub

    'Private Sub SetDecimalToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SetDecimalToolStripMenuItem.Click
    '    'If TabLPanelDecSetting.Visible = True Then
    '    '    SetDecimalToolStripMenuItem.Checked = False
    '    '    TabLPanelDecSetting.Visible = False
    '    'Else
    '    '    SetDecimalToolStripMenuItem.Checked = True
    '    '    TabLPanelDecSetting.Visible = True
    '    'End If
    '    DecimalSetting.ShowForm()
    'End Sub

    Private Sub BtnDecDec_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDecDec.Click

        If div = 1000000000 Then
            div = 100000000
            str = "Ten Crores"
        ElseIf div = 100000000 Then
            div = 10000000
            str = "Crores"
        ElseIf div = 10000000 Then
            div = 1000000
            str = "Ten Lacs"
        ElseIf div = 1000000 Then
            div = 100000
            str = "Lacs"
    ...
    Private Sub BtnIncDec_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnIncDec.Click
        If div = 1 Then
            div = 10
            str = "Tens"
        ElseIf div = 10 Then
    ...
    Private Sub SelectDateToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SelectDateToolStripMenuItem.Click
        Dim FrmDtSel As New FrmDateSelection
        Dim dt As DataTable = FrmDtSel.ShowForm(dttoday.Value.ToString("dd/MMM/yyyy"), dtexp.Value.ToString("dd/MMM/yyyy"), CType(grdact.DataSource, DataTable))
    ...
    Private Sub DecimalSettingToolStripMenuItem1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DecimalSettingToolStripMenuItem1.Click
        DecimalSetting.ShowForm()
    End Sub
    ...
    'Public Sub SearchScenario()
    'Dim DtCompany As New DataTable
    'DtCompany = ObjSceDetail.Select_Scenario()
    'If tbcomp.TabCount > 1 OrElse DtCompany.Rows.Count > 1 Then
    ...
    'MDI.ToolStripcompanyCombo.ComboBox.DisplayMember = "ScenarioName"
    'MDI.ToolStripcompanyCombo.ComboBox.ValueMember = "ScenarioName"
    'End If
    ...
    'End If
    'End Sub
    Public Sub AddUserDefScenario(ByVal sName As String)
        Dim tab As New TabPage
        tab.Text = sName
        tab.Name = sName
        tab.Tag = sName
        tab.ToolTipText = sName

        Dim ObjUserdefTab As New UserDefScenario
        ObjUserdefTab = FrmUserDefScenario.ShowForm(sName)
        Dim strtab As String = ObjUserdefTab.sTagName
        If strtab = "" Then
            Return
        End If
        If ObjUserdefTab.bIsValid = True Then
            ...
            If tbcomp.SelectedTab.Text = tab.Text Then
                If tbcomp.SelectedTab.Text <> ObjUserdefTab.sTagName Then
                    'Update in 
                    '
                    ObjUserdefTab.Update_Scenario(tbcomp.SelectedTab.Text, ObjUserdefTab.sTagName)
                    'UserDef DataTable
                End If
                tbcomp.SelectedTab.Text = ObjUserdefTab.sTagName
                tbcomp.SelectedTab.Name = ObjUserdefTab.sTagName
                tbcomp.SelectedTab.Tag = ObjUserdefTab.sTagName
                tbcomp.SelectedTab.ToolTipText = ObjUserdefTab.sTagName
...
Add:
                ...
                tab.Tag = ObjUserdefTab.sTagName
                tab.ToolTipText = ObjUserdefTab.sTagName
                ...
                ObjUserdefTab.Insert_Scenario(tab.Name, 0, 0, 0, dttoday.Value.Date, dtexp.Value.Date, Val(txtinterval.Text), Val(txtllimit.Text), 0, "Value", "Expiry")
                tbcomp.SelectedTab = tab 'tbcomp.TabPages(ScenarioName)
                'ObjUserdefTab.Insert_Scenario(ScenarioName, Val(txtmkt.Text), Val(txtcvol.Text), Val(txtpvol.Text), dttoday.Value.Date, dtexp.Value.Date, Val(txtinterval.Text), Val(txtllimit.Text), Val(TxtMStrike.Text), "Value", "Expiry")
                TgAddSize = tab.Size.Width
                ScenarioName = tab.Name
    ...
    Private Sub tbcomp_ControlAdded(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ControlEventArgs) Handles tbcomp.ControlAdded
        'PicBoxAdd.Location = New Point(PicBoxAdd.Location.X + 10, PicBoxAdd.Location.Y + 0)
        PicBoxAdd.Location = New Point(PicBoxAdd.Location.X + 90, PicBoxAdd.Location.Y + 0)
    End Sub

    Private Sub tbcomp_ControlRemoved(ByVal sender As Object, ByVal e As System.Windows.Forms.ControlEventArgs) Handles tbcomp.ControlRemoved
        PicBoxAdd.Location = New Point(PicBoxAdd.Location.X - 90, PicBoxAdd.Location.Y + 0)
    End Sub
    ...
    Private Sub RenameToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles RenameToolStripMenuItem.Click
        If tbcomp.SelectedTab.Name = cnstAnalysisScenario Then Exit Sub
        Call AddUserDefScenario(tbcomp.SelectedTab.Name)
        'SearchScenario()
    End Sub

    Private Sub DeleteToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DeleteToolStripMenuItem.Click
        If tbcomp.SelectedTab.Name = cnstAnalysisScenario Then Exit Sub
        If MsgBox("Do you wand to delete selected Scenario.(" & tbcomp.SelectedTab.Name & ")", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            ObjSceDetail.Delete_scenario()
            grdtrad.Rows.Clear()
            tbcomp.Controls.Remove(tbcomp.SelectedTab)
        End If
    End Sub

    Private Sub tbcomp_MarginChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbcomp.MarginChanged
        'MsgBox("MARGIN CHANGE" & tbcomp.SelectedTab.Name)
    End Sub

    Private Sub tbcomp_MouseCaptureChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbcomp.MouseCaptureChanged
        'MsgBox("mOUSECAPTURE " & tbcomp.SelectedTab.Name)
    End Sub

    Private Sub tbcomp_MouseClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles tbcomp.MouseClick
        ' MsgBox("mOUSE CLICK" & tbcomp.SelectedTab.Name)
    End Sub

    Private Sub tbcomp_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles tbcomp.MouseDown
        '  MsgBox("mOUSE dOWN" & tbcomp.SelectedTab.Name)
    End Sub

    Private Sub tbcomp_MouseEnter(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbcomp.MouseEnter
        'MsgBox("mOUSE eNTER" & tbcomp.SelectedTab.Name)
    End Sub

    Private Sub tbcomp_MouseLeave(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbcomp.MouseLeave
        '   MsgBox("mOUSE Leave" & tbcomp.SelectedTab.Name)
    End Sub

    Private Sub tbcomp_Selected(ByVal sender As Object, ByVal e As System.Windows.Forms.TabControlEventArgs) Handles tbcomp.Selected
        '    MsgBox("sELECTED " & tbcomp.SelectedTab.Name)
    End Sub

    Private Sub tbcomp_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles tbcomp.SelectedIndexChanged
        If tbcomp.TabPages.Count > 0 Then
            flgtabchange = True
            ClearallChart()
            LoadTag(tbcomp.SelectedTab.Name)
            flgtabchange = False
            GetSpotDiff(False)
        End If
    End Sub
    ...
    Private Sub LoadTag(ByVal strTag As String)

        ObjSceDetail.select_scenario_Name(strTag)


        ScenarioName = ObjSceDetail.ScenarioName
        txtmkt.Text = ObjSceDetail.CMP
        txtcvol.Text = ObjSceDetail.CVol
        txtpvol.Text = ObjSceDetail.PVol
        dttoday.Value = ObjSceDetail.StartDate
        isStartEndDateChange = True
        dtexp.Value = ObjSceDetail.EndDate
        ...
        If dtexp.Value.Date < dttoday.Value.Date Then
            dtexp.Value = dttoday.Value.Date
        End If
        If ObjSceDetail.interval_type.ToUpper = "PER" Then
            chkint.Checked = True
        ElseIf ObjSceDetail.interval_type.ToUpper = "VALUE" Then
            chkint.Checked = False
        End If
        'IIf(chkint.Checked = True, "Per", "Value") ObjSceDetail.interval_type


        '-==========================================================
        'If ObjSceDetail.interval = 0 Then
        '    If chkint.Checked = True Then
        '        txtinterval.Text = 2
        '    Else
        '        txtinterval.Text = 100
        '    End If
        'Else
        '    txtinterval.Text = ObjSceDetail.interval
        'End If
        REM Suggession By Mohsin Done By Viral
        'chkint.Checked = True
        If chkint.Checked = True Then
            If ObjSceDetail.interval.ToString.Length = 0 Then
                txtinterval.Text = 2
            Else
                txtinterval.Text = ObjSceDetail.interval
            End If
        ...
        txtllimit.Text = ObjSceDetail.strike
        TxtMStrike.Text = ObjSceDetail.MStrike
        grossmtm = ObjSceDetail.PNL
        'Call TabLPanel1_Leave(TabLPanel1, New System.EventArgs)
        ...
        isStartEndDateChange = False
    End Sub
    'Private Sub LoadTag1(ByVal strTag As String)
    ...
    'End Sub
    Private Sub SaveTag(ByVal strtab As String)
        Try
            ...
            ObjSceDetail.ScenarioName = ScenarioName
            ObjSceDetail.CMP = Val(txtmkt.Text)
            ObjSceDetail.CVol = Val(txtcvol.Text)
            ObjSceDetail.PVol = Val(txtpvol.Text)
            ObjSceDetail.StartDate = dttoday.Value.Date
            ObjSceDetail.EndDate = dtexp.Value.Date
            ...
            ObjSceDetail.strike = Val(txtllimit.Text)
            ObjSceDetail.MStrike = Val(TxtMStrike.Text)
            ObjSceDetail.SelectedDate = GetSelectedDate()
            If ObjSceDetail.SelectedDate = "" Then
                ObjSceDetail.SelectedDate = Date.Today
            End If
            ObjSceDetail.PNL = grossmtm
            ...
            For Each grow As DataGridViewRow In grdtrad.Rows
                If grow.Index = grdtrad.Rows.Count - 1 Then Exit For
                Dim drow As DataRow
                drow = dtDetail.NewRow()
                drow("ScenarioName") = ScenarioName

                drow("Flag") = CBool(grow.Cells("Active").Value)
                drow("StartDate") = CDate(grow.Cells("TimeI").Value)
                If grow.Cells("TimeII").Value.ToString() = "" Then
                    drow("Expiry") = CDate(dtexp.Value)
                Else
                    drow("Expiry") = CDate(grow.Cells("TimeII").Value)
                End If



                drow("CPF") = CStr(grow.Cells("CPF").Value)
                drow("Underlying") = Val(grow.Cells("spval").Value)
                drow("Strike") = Val(grow.Cells("Strike").Value)
                drow("Qty") = Val(grow.Cells("units").Value)
                drow("ltp") = Val(grow.Cells("ltp").Value)
                drow("Rate") = Val(grow.Cells("last").Value)
                drow("Vol") = Val(grow.Cells("lv").Value)
                drow("Delta") = Val(grow.Cells("delta").Value)
                drow("DelVal") = Val(grow.Cells("deltaval").Value)
                drow("Theta") = Val(grow.Cells("theta").Value)
                drow("Thval") = Val(grow.Cells("thetaval").Value)
                drow("Vega") = Val(grow.Cells("vega").Value)
                drow("Vgval") = Val(grow.Cells("vgval").Value)
                drow("Gamma") = Val(grow.Cells("gamma").Value)
                drow("Gamval") = Val(grow.Cells("gmval").Value)

                drow("Volga") = Val(grow.Cells("volga").Value)
                drow("Voval") = Val(grow.Cells("volgaval").Value)
                drow("Vanna") = Val(grow.Cells("vanna").Value)
                drow("Vaval") = Val(grow.Cells("vannaval").Value)

                drow("DifFactor") = Val(grow.Cells("DifFactor").Value)
    ...
    Private Sub LoadSelectedDate(ByVal strdate As String)
        Dim str() As String
        str = strdate.Split(",")
        Dim strcol As String
        bCheckAll = True
        If grdact.Rows.Count > 0 Then
            For Each col As DataGridViewColumn In grdact.Columns
                grdact.Rows(0).Cells(col.Index).Value = False
            Next
            Dim dt As New DataTable
            dt = CType(grdact.DataSource, DataTable)
            Try
                For i As Integer = 0 To str.Length - 1
                    strcol = str(i)
                    If strcol.ToUpper = "EXPIRY" Then
                        strcol = strcol & " " & Format(dtexp.Value.Date, "MM/dd/yyyy")
                    End If
                    dt.Rows(0)(strcol) = True
                Next
            Catch ex As Exception
        ...
        End If
    End Sub
    Private Function GetSelectedDate() As String
        ...
        If dt Is Nothing = False Then
            For Each dcol As DataColumn In dt.Columns
                If CBool(IIf(IsDBNull(dt.Rows(0)(dcol.ColumnName)), "False", dt.Rows(0)(dcol.ColumnName))) = True Then
                    result = result & IIf((result.Length > 0), "," & dcol.ColumnName, dcol.ColumnName)
                End If
            Next
        End If
        ...
        Return result
    End Function
    Private Sub LoadScenarioDetail()
        ...
        Try
            ...
            Dim tempdata As New DataTable
            tempdata = ObjSceDetail.dtScenarioDetail
            grdtrad.Refresh()
            grdtrad.RefreshEdit()
            ...
            '    grdtrad.AllowUserToDeleteRows = True
            '    For i = 1 To grdtrad.RowCount - 1
            '        Try
            '            grdtrad.Rows.Remove(grdtrad.Rows(i - 1))
            '            grdtrad.Refresh()
            '        Catch ex As Exception
            ...
            If tempdata.Rows.Count > 0 Then
                ...
                Dim i As Integer = 0
                For Each drow In tempdata.Rows
                    Dim ObjVal(38) As Object
                    ObjVal(gc.Flag) = drow("Flag")
                    ObjVal(gc.StartDate) = drow("StartDate")
                    ObjVal(gc.Expiry) = drow("Expiry")
                    ObjVal(gc.CPF) = drow("CPF")
                    ObjVal(gc.Underlying) = drow("Underlying")
                    ObjVal(gc.Strike) = drow("Strike")
                    ObjVal(gc.Qty) = drow("Qty")
                    ObjVal(gc.LTP) = drow("LTP")
                    ObjVal(gc.Vol) = drow("Vol")
                    ObjVal(gc.VolORG) = drow("VolORG")
                    ObjVal(gc.Rate) = drow("Rate")
                    ObjVal(gc.Delta) = drow("Delta")
                    ObjVal(gc.DelVal) = drow("DelVal")
                    ObjVal(gc.Gamma) = drow("Gamma")
                    ObjVal(gc.GamVal) = drow("GamVal")
                    ObjVal(gc.Vega) = drow("Vega")
                    ObjVal(gc.VgVal) = drow("VgVal")
                    ObjVal(gc.Theta) = drow("Theta")
                    ObjVal(gc.ThVal) = drow("ThVal")
                    ObjVal(gc.Volga) = drow("Volga")
                    ObjVal(gc.VoVal) = drow("VoVal")
                    ObjVal(gc.Vanna) = drow("Vanna")
                    ObjVal(gc.VaVal) = drow("VaVal")
                    ObjVal(gc.UID) = drow("UID")
                    ObjVal(gc.DelVal1) = drow("DelVal1")
                    ObjVal(gc.GamVal1) = drow("GamVal1")
                    ObjVal(gc.VgVal1) = drow("VgVal1")
                    ObjVal(gc.ThVal1) = drow("ThVal1")
                    ObjVal(gc.VoVal1) = drow("VoVal1")
                    ObjVal(gc.VaVal1) = drow("VaVal1")
                    ObjVal(gc.LTP1) = drow("LTP1")
                    ObjVal(gc.Delta1) = drow("Delta1")
                    ObjVal(gc.Gamma1) = drow("Gamma1")
                    ObjVal(gc.Vega1) = drow("Vega1")
                    ObjVal(gc.Theta1) = drow("Theta1")
                    ObjVal(gc.Volga1) = drow("Volga1")
                    ObjVal(gc.Vanna1) = drow("Vanna1")
                    ObjVal(gc.DifFactor) = drow("DifFactor")
                    ObjVal(gc.ScenarioName) = drow("ScenarioName")

                    grdtrad.Rows.Insert(i, ObjVal)
                ...
                Next
            End If
            'grdtrad.DataSource = tempdata
        Catch ex As Exception
            MsgBox(ex.Message & vbCrLf & "LoadScenarioDetail")
            ' MsgBox(ex.ToString)
        End Try
    ...
    Private Sub grdtrad_RowValidating(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles grdtrad.RowValidating

    End Sub

    Private Sub grdtrad_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles grdtrad.Validating
        SaveTag(ScenarioName)
    End Sub

    Private Sub txtpvol_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtpvol.TextChanged
        Flgtxtpvol = True
    End Sub
    ...
    Private Sub txtinterval_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Sub ShowForm(ByVal p1 As Boolean)
        '   Throw New NotImplementedException
    End Sub

    Private Sub chkint_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkint.Leave
    ...
    Private Sub TxtNoDate_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtNoDate.Leave
        CType(sender, TextBox).BackColor = Color.FromArgb(255, 255, 192)

        SCE_DAYS = TxtNoDate.Text
        trd.Update_setting(TxtNoDate.Text, "SCE_DAYS ")
        create_active()
    End Sub



    Private Sub txtllimit_TextChanged(sender As Object, e As EventArgs) Handles txtllimit.TextChanged
        'If txtllimit.Text > 50 Then
        '    Dim result As DialogResult = MessageBox.Show("We recommend setting the strike below 50 for better performance of Volhedge. Do you still want to continue with your value?", "Performance Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning)

        '    If result = DialogResult.Cancel Then
        '        txtllimit.Text = "50" ' Set interval to 0.5 if the user clicks Cancel
        '    End If
        'End If
    End Sub

    Private Sub txtllimit_Layout(sender As Object, e As LayoutEventArgs) Handles txtllimit.Layout

    End Sub

    Private Sub txtllimit_Leave_1(sender As Object, e As EventArgs) Handles txtllimit.Leave
        If txtllimit.Text > 50 Then
            Dim result As DialogResult = MessageBox.Show("We recommend setting the strike below 50 for better performance of Volhedge. Do you still want to continue with your value?", "Performance Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning)
    ...
    Private Sub CMnuStrpTabLPanel2_Opening(sender As Object, e As System.ComponentModel.CancelEventArgs) Handles CMnuStrpTabLPanel2.Opening

    End Sub

    Private Sub chartPNL_DoubleClick(sender As Object, e As EventArgs)
        Dim chart As ScottPlot.WinForms.FormsPlot = sender
        chartName = chart.Tag
        objProfitLossChart.ShowForm()
    End Sub
