Public Class rptProfitLossDaywise
    ...
    Private Sub fill_table()
        ...
        For Each frow As DataRow In dtScript.Rows
            ...
            If frow("script").ToString.Substring(0, 3) = "FUT" Then
                stexp = Val(foTable.Compute("sum(tot)", "cp not in ('C','P') and script='" & frow("script") & "' and qty > 0 and entry_date = #" & fDate(frow("entry_date")) & "# And company='" & frow("company") & "'").ToString)
                stexp1 = Math.Abs(Val(foTable.Compute("sum(tot)", "cp not in ('C','P') and script='" & frow("script") & "' and qty < 0 and entry_date = #" & fDate(frow("entry_date")) & "# And company='" & frow("company") & "'").ToString))
                exp += ((stexp * futl) / futlp)
                exp += ((stexp1 * futs) / futsp)
            ElseIf frow("script").ToString.Substring(0, 3) = "OPT" Then
                'Option ####################################################################
                If spl <> 0 Then
                    stexp = Val(foTable.Compute("sum(tot)", "cp in ('C','P') and script='" & frow("script") & "' and qty > 0 and entry_date = #" & fDate(frow("entry_date")) & "# And company='" & frow("company") & "'").ToString)
                    stexp1 = Math.Abs(Val(foTable.Compute("sum(tot)", "cp  in ('C','P') and script='" & frow("script") & "' and qty < 0 and entry_date = #" & fDate(frow("entry_date")) & "# And company='" & frow("company") & "'").ToString))
                    exp += ((stexp * spl) / splp)
                    exp += ((stexp1 * sps) / spsp)
                Else
                    stexp = Val(foTable.Compute("sum(tot)", "cp  in ('C','P') and script='" & frow("script") & "' and qty > 0 and entry_date = #" & fDate(frow("entry_date")) & "# And company='" & frow("company") & "'").ToString)
        ...
        For Each frow As DataRow In dtScript.Rows
            ...
            exp = 0
            stexp = Math.Round(Val(eqTable.Compute("sum(tot)", "script='" & frow("script") & "' and qty > 0 and entry_date = #" & fDate(frow("entry_date")) & "# And company='" & frow("company") & "'").ToString), 2)
            stexp1 = Math.Abs(Math.Round(Val(eqTable.Compute("sum(tot)", "script='" & frow("script") & "' and qty < 0 and entry_date = #" & fDate(frow("entry_date")) & "# And company='" & frow("company") & "'").ToString), 2))
            dst = stexp - stexp1
            If dst > 0 Then
                ndst = stexp1
    ...
    Private Sub rptProfitLossDaywise_KeyUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyUp
