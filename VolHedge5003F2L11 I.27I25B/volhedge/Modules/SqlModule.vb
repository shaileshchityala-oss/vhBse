...
Module SqlModule
    ...
    'Public FoTokens As New Hashtable
    'Public EqTokens As New Hashtable
    'Public CurTokens As New Hashtable
    ...
    Public Sub process_fo_SQL(ByVal obj As Object)
        Try
            ...
            If DAL.DA_SQL._conFo.State = ConnectionState.Open Then DAL.DA_SQL._conFo.Close()
            ...
            While Dr.Read()
                Try
                    ...
                    '   WriteLogbastlog_ltp("token_no=" + token_no.ToString() + "last_trade_price=" + last_trade_price.ToString() + "BDate=" + DateTime.Now.ToString())
    ...
    Public Function select_exposure_p() As DataTable
        Try
            ...
            'Cmd2 = New SqlClient.SqlCommand("dbo.select_exposure_p", DAL.DA_SQL._conFo)
            ...
            Dim sp_exposure As String = "dbo.select_exposure_p"
            Dim DT As New DataTable
            FinAccessSQL.ParamClear()

            FinAccessSQL.Cmd_Text = sp_exposure

            DT = FinAccessSQL.FillList()
    ...
    End Function
    Public Function Get_span_data(ByVal strspan As String) As DataTable

        '  Dim sqlcon As CSqlConnection
        '  sqlcon = New CSqlConnection(CSqlConnection.mStr)

        Try

            '  If DAL.DA_SQL._conFo.State = ConnectionState.Closed Then DAL.DA_SQL.open_Fo_connection()

            'Dim Cmd_span = New SqlClient.SqlCommand
            'Dim Dr_span As SqlClient.SqlDataReader

            'Cmd_span = New SqlClient.SqlCommand("dbo.get_span_output '" & strspan & "'", DAL.DA_SQL._conFo)


            'Dr_span = Cmd_span.ExecuteReader()
            'Dim dt As New DataTable
            'dt.Load(Dr_span)
            'Return dt


            'Dim Da As New SqlClient.SqlDataAdapter("dbo.get_span_output '" & strspan & "'", DAL.DA_SQL._conFo)
            'Dim Dt As DataTable = New DataTable()
            'Da.Fill(dt)
            'Return Dt

            '    sqlcon.ParamClear()
            '    sqlcon.AddParam("@strspan", SqlDbType.NVarChar, 1000, strspan)
            '   Dim dtTest As DataTable = sqlcon.FillDatatable("Sp_getmargin")


            '    clsGlobal.mPerf.SetFileName("SqlFillSQlCnt")
            '    clsGlobal.mPerf.Write_Counter("DataFillSQL")
            '    clsGlobal.mPerf.WriteLogStr("SQLSeverFill : " + FinAccessSQL._con.ConnectionString)


            Dim Sp_getmargin As String = "dbo.get_span_output"
            Dim DT As New DataTable
            'Dim ddt As New DataTable
            'If ConState = "SQLCON" Then
            FinAccessSQL.ParamClear()
            FinAccessSQL.AddParam("@strspan", SqlDbType.NVarChar, 1000, strspan)
            'If iscurr = True Then
            '    MyAccessSQL.Cmd_Text = Sp_getcurrmargin
            'Else
            FinAccessSQL.Cmd_Text = Sp_getmargin
            ' End If
    ...
    Public Function process_MWData_SQL(ByVal strSymbol As String, ByVal strMDate As String, ByVal iSkip As Integer, ByVal iUpDownStrike As Integer) As DataTable
        ...
        Try
            ...
            Try
                If bool_IsTelNet = False Then CheckTelNet()
                If bool_IsTelNet = True Then
                    'Cmd = New SqlClient.SqlCommand("dbo.SP_GetMWData '" & strSymbol & "', '" & strMDate & "', '" & iSkip.ToString() & "'", DAL.DA_SQL._conFo)

                    '   Cmd.Connection = DAL.DA_SQL._conFo
                    '1Sp_023_New 
                    'Dr = Cmd.ExecuteReader()
                    'Da.SelectCommand = Cmd
                    Dim Da As New SqlClient.SqlDataAdapter("dbo.SP_GetMWData '" & strSymbol & "', '" & strMDate & "','" & iSkip.ToString() & "', '" & iUpDownStrike.ToString() & "'", DAL.DA_SQL._conFo)
                    Dim Dt As DataTable = New DataTable()
                    Da.Fill(Dt)
                    Return Dt
                End If
    ...
    Public Function process_MWData_SQL1(ByVal strSymbol As String, ByVal strMDate As String, ByVal iSkip As Integer, ByVal iUpDownStrike As Integer) As DataTable

        'SyncLock SyncLock1

        Try
            'Monitor.Enter(objLock1)
            If DAL.DA_SQL._conFo.State = ConnectionState.Closed Then DAL.DA_SQL.open_Fo_connection()
            Try
                If bool_IsTelNet = False Then CheckTelNet()
                If bool_IsTelNet = True Then
        ...
        Catch ex As Exception
            ...
            Return Nothing
            'Finally
            'Monitor.Exit(objLock1)
        End Try

    End Function

    Public Function process_MWData_SQL2(ByVal strSymbol As String, ByVal strMDate As String, ByVal iSkip As Integer, ByVal iUpDownStrike As Integer) As DataTable
