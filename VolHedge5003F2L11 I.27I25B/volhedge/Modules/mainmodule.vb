...
lblAppCrash:
    ...
    Private Sub fill_equity()
        Try
            ...
            '            dr("script") = drow("script").ToString.Trim
            '            dr("company") = drow("company").ToString.Trim
            '            dr("eq") = CStr(drow("eq"))
            '            dr("prqty") = Val(GdtEQTrades.Compute("sum(qty)", "script='" & drow("script") & "' and entry_date < #" & fDate(Now.Date) & "# And company='" & drow("company") & "'").ToString)
            '            dr("toqty") = Val(GdtEQTrades.Compute("sum(qty)", "script='" & drow("script") & "' and entry_date >= #" & fDate(Now.Date) & "# And company='" & drow("company") & "'").ToString)
            '            dr("qty") = Val(GdtEQTrades.Compute("sum(qty)", "script='" & drow("script") & "' And company='" & drow("company") & "'"))
    ...
    '                If dtfo.Rows.Count = 0 Then
    '                    objTrad.select_FOHTable(dtfo, drow("company"))
    '                End If

    '                If dtfo.Select("company='" & drow("company") & "' ").Length = 0 Then
    '                    objTrad.select_FOHTable(dtfo, drow("company"))
    '                End If
    '                If dtsrate.Rows.Count = 0 Then
    '                    dtsrate = New DataView(GdtFOTrades, "script='" & drow("script") & "' and tot < 0 And company='" & drow("company") & "'", "", DataViewRowState.CurrentRows).ToTable
    '                End If
    '                If dtsrate.Select("company='" & drow("company") & "' ").Length = 0 Then
    '                    dtsrate = New DataView(GdtFOTrades, "script='" & drow("script") & "' and tot < 0 And company='" & drow("company") & "'", "", DataViewRowState.CurrentRows).ToTable
    '                End If
    '                If dtbrate.Rows.Count = 0 Then
    '                    dtbrate = New DataView(GdtFOTrades, "script='" & drow("script") & "' and tot > 0 And company='" & drow("company") & "'", "", DataViewRowState.CurrentRows).ToTable
    '                End If
    '                If dtbrate.Select("company='" & drow("company") & "' ").Length = 0 Then
    '                    dtbrate = New DataView(GdtFOTrades, "script='" & drow("script") & "' and tot > 0 And company='" & drow("company") & "'", "", DataViewRowState.CurrentRows).ToTable
    '                End If
    ...
    '                        Dim srate As Double = 0
    '                        dr = dtable.NewRow()
    '                        dr("strikes") = Val(drow("strikerate").ToString)


    '                        dr("prqty") = Val(dtfo.Select("Script='" & drow("Script") & "'")(0).Item("PrevQty"))

    '                        '   dr("prqty") = Val(GdtFOTrades.Compute("sum(qty)", "script='" & drow("script") & "' and entry_date < #" & Format(Today, "dd-MMM-yyyy") & "# And company='" & drow("company") & "'").ToString)

    '                        dr("toqty") = Val(dtfo.Select("Script='" & drow("Script") & "'")(0).Item("TodayQty"))

    '                        '  dr("toqty") = Val(GdtFOTrades.Compute("sum(qty)", "script='" & drow("script") & "' and entry_date >= #" & Format(Today, "dd-MMM-yyyy") & "# And company='" & drow("company") & "'").ToString)

    '                        dr("units") = dr("prqty") + dr("toqty")

    '                        '    dr("units") = Val(GdtFOTrades.Compute("sum(qty)", "script='" & drow("script") & "' And company='" & drow("company") & "'"))

    '                        srate = Math.Abs(Val(dtsrate.Compute("sum(tot)", "script='" & drow("script") & "' and tot < 0 And company='" & drow("company") & "'").ToString))
    '                        brate = Val(dtbrate.Compute("sum(tot)", "script='" & drow("script") & "' and tot > 0 And company='" & drow("company") & "'").ToString)
    '                        ' dr("traded") = 0
    ...
    '                        dr("deltaval") = 0
    '                        dr("deltaval1") = 0
    '                        If Not drow("cp").ToString = "" Then
    '                            If drow("cp") = "C" Or drow("CP") = "P" Then
    '                                dr("cp") = CStr(drow("cp"))
    '                            Else
    '                                dr("cp") = "F"
    '                                dr("delta") = 1
    ...
    '                        prExp = 0
    '                        toExp = 0
    '                        Call cal_position_expense(drow, prExp, toExp, "FO")
    ...
    '                            dr("preDate") = CDate(IIf(IsDBNull(DtAnalysis.Compute("Max(PreDate)", "tokanno=" & dr("tokanno") & " And company='" & dr("company") & "'")) = True, Today, DtAnalysis.Compute("Max(PreDate)", "tokanno=" & dr("tokanno") & " And company='" & dr("company") & "'")))
    ...
    '                        'dr("prqty") = Val(GdtFOTrades.Compute("sum(qty)", "script='" & drow("script") & "' and entry_date < #" & fDate(Now.Date) & "# And company='" & drow("company") & "'").ToString)
    '                        'dr("toqty") = Val(GdtFOTrades.Compute("sum(qty)", "script='" & drow("script") & "' and entry_date >= #" & fDate(Now.Date) & "# And company='" & drow("company") & "'").ToString)
    ...
    '                        dr("units") = Val(drow("qty").ToString)
    '                        dr("traded") = Math.Round(Val(drow("rate").ToString), 2)
    '                        dr("last") = 0
    '                        dr("lv") = 0
    '                        dr("MktVol") = 0
    '                        dr("delta") = 0
    ...
    '                        dr("isliq") = CBool(drow("isliq"))
    '                        '===========================================keval
    '                        dr("asset_tokan") = 0 'IIf(IsDBNull(scripttable.Compute("max(asset_tokan)", "script='" & drow("script") & "'")), 0, scripttable.Compute("max(asset_tokan)", "script='" & drow("script") & "'"))
    '                        '=========================================
    '                        dr("tokanno") = Val(cpfmaster.Compute("max(token)", "script='" & drow("script") & "'").ToString)
    ...
    '                        dr("units") = Val(GdtFOTrades.Compute("sum(qty)", "script='" & drow("script") & "' And company='" & drow("company") & "'"))
    '                        srate = Math.Abs(Val(GdtFOTrades.Compute("sum(tot)", "script='" & drow("script") & "' and tot < 0 And company='" & drow("company") & "'").ToString))
    '                        brate = Val(GdtFOTrades.Compute("sum(tot)", "script='" & drow("script") & "' and tot > 0 And company='" & drow("company") & "'").ToString)
    '                        ' dr("traded") = 0
    '                        If Val(dr("units")) = 0 Then
    '                            dr("traded") = Val((brate - srate))
    '                        Else
